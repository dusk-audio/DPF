...
/**
   @mainpage DAF %Plugin Framework

   DAF %Plugin Framework (or @b DAF for short)
   ...
   DAF can build for LADSPA, DSSI, LV2, VST2, VST3 and CLAP formats.@n
   A JACK/Standalone mode is also available, allowing you to quickly test plugins.

   @section Macros
   You start by creating a "DafPluginInfo.h" file describing the plugin via macros, see @ref PluginMacros.@n
   This file is included during compilation of the main DAF code to select which features to activate for each plugin format.
   ...
   @section Plugin
   The next step is to create your plugin code by subclassing DAF's Plugin class.@n
   You need to pass the number of parameters in the constructor and also the number of programs and states, if any.

   Do note all of DAF code is within its own C++ namespace (@b DAF for DSP/plugin stuff, @b DGL for UI stuff).@n
   You can use @ref START_NAMESPACE_DAF / @ref END_NAMESPACE_DAF combo around your code, or globally set @ref USE_NAMESPACE_DAF.@n
   ...
   /* Make DAF related classes available for us to use without any extra namespace references */
   USE_NAMESPACE_DAF;

   /**
      Our custom plugin class.
      Subclassing `Plugin` from DAF is how this all works.
   ...
   /**
      ...
      This is the entry point for DAF plugins.
      DAF will call this to either create an instance of your plugin for the host or to fetch some initial information for internal caching.
    */
   Plugin* createPlugin()
   {
   ...
   In DAF parameters can be inputs or outputs.@n
   ...
   @section Programs
   Programs in DAF refer to plugin-side presets (usually called "factory presets").@n
   This is meant as an initial set of presets provided by plugin authors included in the actual plugin.

   To use programs you must first enable them by setting @ref DAF_PLUGIN_WANT_PROGRAMS to 1 in your DafPluginInfo.h file.@n
...
/**
   ...
   C Macros that describe your plugin. (defined in the "DafPluginInfo.h" file)
   ...
   Only 4 macros are required, they are:
    - @ref DAF_PLUGIN_NAME
    - @ref DAF_PLUGIN_NUM_INPUTS
    - @ref DAF_PLUGIN_NUM_OUTPUTS
    - @ref DAF_PLUGIN_URI
   
   Additionally, @ref DAF_PLUGIN_CLAP_ID is required if building CLAP plugins.
   @{
 */
 ...
 */
#define DAF_PLUGIN_NAME "Plugin Name"
...
/**
   ...
   @note This macro is required.
 */
#define DAF_PLUGIN_URI "urn:daf:name"

/**
   Whether the plugin has a custom %UI.
   @see DAF_UI_USE_NANOVG
   @see UI
 */
...
/**
   ...
   @ref DAF_PLUGIN_WANT_MIDI_INPUT is automatically enabled when this is too.
   @see DAF_PLUGIN_WANT_MIDI_INPUT
 */
...
/**
   ...
   This is automatically enabled if @ref DAF_PLUGIN_IS_SYNTH is true.
 */
 ...
 */
#define DAF_PLUGIN_WANT_MIDI_OUTPUT 1
 ...
 */
#define DAF_PLUGIN_WANT_PARAMETER_VALUE_CHANGE_REQUEST 1
 ...
 */
#define DAF_UI_USE_CAIRO 1

/**
   Whether the %UI uses a custom toolkit implementation based on OpenGL.@n
   When enabled, the macros @ref DAF_UI_CUSTOM_INCLUDE_PATH and @ref DAF_UI_CUSTOM_WIDGET_TYPE are required.
 */
#define DAF_UI_USE_CUSTOM 1

/**
   The include path to the header file used by the custom toolkit implementation.
   This path must be relative to dpf/daf/DafUI.hpp
   @see DAF_UI_USE_CUSTOM
 */
#define DAF_UI_CUSTOM_INCLUDE_PATH
...
/**
   ...
   @see DAF_UI_USE_CUSTOM
 */
#define DAF_UI_CUSTOM_WIDGET_TYPE
...
/**
   ...
   When this macro is defined, the companion DAF_UI_DEFAULT_HEIGHT macro must be defined as well.
 */
#define DAF_UI_DEFAULT_WIDTH 300
...
/**
   ...
   When this macro is defined, the companion DAF_UI_DEFAULT_WIDTH macro must be defined as well.
 */
#define DAF_UI_DEFAULT_HEIGHT 300
...
/**
   ...
   By default this is set to @ref DAF_PLUGIN_URI with "#UI" as suffix.
 */
#define DAF_UI_URI DAF_PLUGIN_URI "#UI"

/**
   The AudioUnit type for a plugin.@n
   This is a 4-character symbol, automatically set by DAF based on other plugin macros.
   See https://developer.apple.com/documentation/audiotoolbox/1584142-audio_unit_types for more information.
 */
#define DAF_PLUGIN_AU_TYPE aufx
...
/**
   ...
   @note Setting this macro will change the uid of a VST3 plugin.
         If you already released a DAF-based VST3 plugin make sure to also enable DAF_VST3_DONT_USE_BRAND_ID
 */
 ...
 */
#define DAF_PLUGIN_UNIQUE_ID test
 ...
 */
#define DAF_PLUGIN_LV2_CATEGORY "lv2:Plugin"
...
/**
   ...
   And extra categories possible for any plugin type:
      ...
      - Stereo
 */
#define DAF_PLUGIN_VST3_CATEGORIES "Fx|Stereo"
...
/**
   ...
   And finally the following audio capabilities can be set:
      ...
      - ambisonic
*/
#define DAF_PLUGIN_CLAP_FEATURES "audio-effect", "stereo"

/**
   The plugin id when exporting in CLAP format, in reverse URI form.
   @note This macro is required when building CLAP plugins
*/
#define DAF_PLUGIN_CLAP_ID "studio.kx.daf.effect"
...
/**
   ...
   C Macros to customize DAF behaviour.

   These are macros that do not set plugin features or information, but instead change DAF internals.
 ...
 */
#define DAF_VST_SHOW_PARAMETER_OUTPUTS

/**
   Forcibly ignore DAF_PLUGIN_BRAND_ID for VST3 plugins.@n
   This is required for DAF-based VST3 plugins that got released without setting DAF_PLUGIN_BRAND_ID first.
 */
...
/**
   ...
   C Macros to use and customize DAF namespaces.
...
/**
   Compiler macro that sets the C++ namespace for DAF plugins.@n
   If unset during build, it will use the name @b DAF by default.

   Unless you know exactly what you are doing, you do need to modify this value.@n
   The only probable useful case for customizing it is if you are building a big collection of very similar DAF-based plugins in your application.@n
   ...
   DAF will automatically use the plugin name as prefix to flat namespace functions in order to avoid conflicts.

   So, basically, it is DAF's job to make sure plugin binaries are 100% usable as-is.@n
   You typically do not need to care about this at all.
 */
#define DAF_NAMESPACE DAF

/**
   Compiler macro that begins the C++ namespace for @b DAF, as needed for (the DSP side of) plugins.@n
   All classes in DAF are within this namespace except for UI/graphics stuff.
   @see END_NAMESPACE_DAF
 */
#define START_NAMESPACE_DAF namespace DAF_NAMESPACE {

/**
   Close the namespace previously started by @ref START_NAMESPACE_DAF.@n
   This doesn't really need to be a macro, it is just prettier/more consistent that way.
 */
#define END_NAMESPACE_DAF }

/**
   Make the @b DAF namespace available in the current function scope.@n
...
/* TODO
 *
 * DAF_MACRO_AS_STRING_VALUE
 * DAF_MACRO_AS_STRING
 * DAF_PROPER_CPP11_SUPPORT
 * DONT_SET_USING_DAF_NAMESPACE
 *
 */
