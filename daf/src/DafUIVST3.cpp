...
# define DAF_VST3_USING_HOST_RUN_LOOP 1
#else
# define DAF_VST3_USING_HOST_RUN_LOOP 0
#endif

#ifndef DAF_VST3_TIMER_INTERVAL
# define DAF_VST3_TIMER_INTERVAL 16 /* ~60 fps */
#endif
 ...
 */
#if !DAF_VST3_USING_HOST_RUN_LOOP
...
public:
    ...
    {
        DAF_SAFE_ASSERT_RETURN(!fCallbackRegistered,);
        fCallbackRegistered = true;
        fUI.addIdleCallbackForNativeIdle(this, DAF_VST3_TIMER_INTERVAL);
    }
...
public:
    ...
    {
        DAF_SAFE_ASSERT_INT_RETURN(keychar >= 0 && keychar < 0x7f, keychar, V3_FALSE);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(point != nullptr,);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fConnection != nullptr,);
    ...
    v3_result notify(v3_message** const message)
    {
        ...
        {
            DAF_SAFE_ASSERT_RETURN(! fReadyForPluginData, V3_INTERNAL_ERR);
        ...
        if (std::strcmp(msgid, "parameter-set") == 0)
        {
            ...
            if (rindex < kVst3InternalParameterBaseCount)
            {
                ...
                case kVst3InternalParameterSampleRate:
                    DAF_SAFE_ASSERT_RETURN(value >= 0.0, V3_INVALID_ARG);
...
private:
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fView != nullptr,);
        DAF_SAFE_ASSERT_RETURN(fFrame != nullptr,);
...
// --------------------------------------------------------------------------------------------------------------------
// daf_ui_connection_point

struct daf_ui_connection_point : v3_connection_point_cpp {
    std::atomic_int refcounter;
    ScopedPointer<UIVst3>& uivst3;
    v3_connection_point** other;

    daf_ui_connection_point(ScopedPointer<UIVst3>& v)
        : refcounter(1),
          uivst3(v),
          other(nullptr)
    {
        ...
        ref = daf_single_instance_ref<daf_ui_connection_point>;
        unref = daf_single_instance_unref<daf_ui_connection_point>;
    ...
    {
        daf_ui_connection_point* const point = *static_cast<daf_ui_connection_point**>(self);
    ...
    {
        daf_ui_connection_point* const point = *static_cast<daf_ui_connection_point**>(self);
        d_debug("UI|daf_ui_connection_point::connect => %p %p", self, other);
    ...
    {
        d_debug("UI|daf_ui_connection_point::disconnect => %p %p", self, other);
        daf_ui_connection_point* const point = *static_cast<daf_ui_connection_point**>(self);

        DAF_SAFE_ASSERT_RETURN(point->other != nullptr, V3_INVALID_ARG);
        DAF_SAFE_ASSERT(point->other == other);
    ...
    static v3_result V3_API notify(void* const self, v3_message** const message)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(uivst3 != nullptr, V3_NOT_INITIALIZED);
...
// --------------------------------------------------------------------------------------------------------------------
// daf_plugin_view_content_scale

struct daf_plugin_view_content_scale : v3_plugin_view_content_scale_cpp {
    std::atomic_int refcounter;
    ScopedPointer<UIVst3>& uivst3;
    // cached values
    float scaleFactor;

    daf_plugin_view_content_scale(ScopedPointer<UIVst3>& v)
        : refcounter(1),
          uivst3(v),
          scaleFactor(0.0f)
    {
        ...
        ref = daf_single_instance_ref<daf_plugin_view_content_scale>;
        unref = daf_single_instance_unref<daf_plugin_view_content_scale>;
    ...
    {
        daf_plugin_view_content_scale* const scale = *static_cast<daf_plugin_view_content_scale**>(self);
    ...
    {
        daf_plugin_view_content_scale* const scale = *static_cast<daf_plugin_view_content_scale**>(self);
        d_debug("daf_plugin_view::set_content_scale_factor => %p %f", self, factor);
...
struct daf_timer_handler : v3_timer_handler_cpp {
    std::atomic_int refcounter;
    ScopedPointer<UIVst3>& uivst3;
    bool valid;

    daf_timer_handler(ScopedPointer<UIVst3>& v)
        : refcounter(1),
          uivst3(v),
          valid(true)
    {
        ...
        ref = daf_single_instance_ref<daf_timer_handler>;
        unref = daf_single_instance_unref<daf_timer_handler>;
    ...
    {
        daf_timer_handler* const timer = *static_cast<daf_timer_handler**>(self);
    ...
    static void V3_API on_timer(void* self)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(timer->valid,);
...
struct daf_plugin_view : v3_plugin_view_cpp {
    std::atomic_int refcounter;
    ScopedPointer<daf_ui_connection_point> connection;
    ScopedPointer<daf_plugin_view_content_scale> scale;
   #if DAF_VST3_USING_HOST_RUN_LOOP
    ScopedPointer<daf_timer_handler> timer;
   #endif
    ScopedPointer<UIVst3> uivst3;
    // cached values
    ...
    daf_plugin_view(v3_host_application** const host, void* const instance, const double sr)
        : refcounter(1),
    ...
    {
        d_debug("daf_plugin_view() with hostApplication %p", hostApplication);
    ...
    {
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
        ...
        if (v3_tuid_match(v3_connection_point_iid, iid))
        {
            ...
            if (view->connection == nullptr)
                view->connection = new daf_ui_connection_point(view->uivst3);
            else
        ...
        if (v3_tuid_match(v3_plugin_view_content_scale_iid, iid))
        {
            ...
            if (view->scale == nullptr)
                view->scale = new daf_plugin_view_content_scale(view->uivst3);
            else
    ...
    static uint32_t V3_API ref_view(void* self)
    {
        ...
        d_debug("daf_plugin_view::ref => %p | refcount %i", self, refcount);
        return refcount;
    }

    static uint32_t V3_API unref_view(void* self)
    {
        daf_plugin_view** const viewptr = static_cast<daf_plugin_view**>(self);
        daf_plugin_view* const view = *viewptr;

        if (const int refcount = --view->refcounter)
        {
            d_debug("daf_plugin_view::unref => %p | refcount %i", self, refcount);
            return refcount;
        }
        ...
        if (daf_ui_connection_point* const conn = view->connection)
        {
            if (const int refcount = conn->refcounter)
            {
                unclean = true;
                d_stderr("DAF warning: asked to delete view while connection point still active (refcount %d)", refcount);
            }
        }

       #ifndef DAF_OS_MAC
        if (daf_plugin_view_content_scale* const scale = view->scale)
        {
            if (const int refcount = scale->refcounter)
            {
                unclean = true;
                d_stderr("DAF warning: asked to delete view while content scale still active (refcount %d)", refcount);
            }
        }
       #endif
        ...
        d_debug("daf_plugin_view::unref => %p | refcount is zero, deleting everything now!", self);
    ...
    {
        d_debug("daf_plugin_view::is_platform_type_supported => %p %s", self, platform_type);
    ...
    {
        d_debug("daf_plugin_view::attached => %p %p %s", self, parent, platform_type);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
        DAF_SAFE_ASSERT_RETURN(view->uivst3 == nullptr, V3_INVALID_ARG);
        ...
        for (size_t i=0; i<ARRAY_SIZE(kSupportedPlatforms); ++i)
        {
            if (std::strcmp(kSupportedPlatforms[i], platform_type) == 0)
            {
                ...
                DAF_SAFE_ASSERT_RETURN(view->frame != nullptr, V3_INVALID_ARG);

                v3_run_loop** runloop = nullptr;
                v3_cpp_obj_query_interface(view->frame, v3_run_loop_iid, &runloop);
                DAF_SAFE_ASSERT_RETURN(runloop != nullptr, V3_INVALID_ARG);
    ...
    {
        d_debug("daf_plugin_view::removed => %p", self);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
        DAF_SAFE_ASSERT_RETURN(view->uivst3 != nullptr, V3_INVALID_ARG);
    ...
    {
        d_debug("daf_plugin_view::on_wheel => %p %f", self, distance);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
    ...
    {
        d_debug("daf_plugin_view::on_key_down => %p %i %i %i", self, key_char, key_code, modifiers);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
    ...
    {
        d_debug("daf_plugin_view::on_key_up => %p %i %i %i", self, key_char, key_code, modifiers);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
    ...
    {
        d_debug("daf_plugin_view::get_size => %p", self);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);

        if (UIVst3* const uivst3 = view->uivst3)
            return uivst3->getSize(rect);

        d_debug("daf_plugin_view::get_size => %p | NOTE: size request before attach", self);
        ...
        if (d_isZero(scaleFactor))
            scaleFactor = 1.0;
        rect->right = DAF_UI_DEFAULT_WIDTH * scaleFactor;
        rect->bottom = DAF_UI_DEFAULT_HEIGHT * scaleFactor;
       #else
    ...
    {
        d_debug("daf_plugin_view::on_size => %p {%d,%d,%d,%d}",
                self, rect->top, rect->left, rect->right, rect->bottom);
        DAF_SAFE_ASSERT_INT2_RETURN(rect->right > rect->left, rect->right, rect->left, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_INT2_RETURN(rect->bottom > rect->top, rect->bottom, rect->top, V3_INVALID_ARG);
    ...
    {
        d_debug("daf_plugin_view::on_focus => %p %u", self, state);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
    ...
    {
        d_debug("daf_plugin_view::set_frame => %p %p", self, frame);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
    ...
    {
        d_debug("daf_plugin_view::check_size_constraint => %p {%d,%d,%d,%d}",
                self, rect->top, rect->left, rect->right, rect->bottom);
        daf_plugin_view* const view = *static_cast<daf_plugin_view**>(self);
...
// --------------------------------------------------------------------------------------------------------------------
// daf_plugin_view_create (called from plugin side)

v3_plugin_view** daf_plugin_view_create(v3_host_application** host, void* instancePointer, double sampleRate);

v3_plugin_view** daf_plugin_view_create(v3_host_application** const host,
                                        void* const instancePointer,
                                        const double sampleRate)
{
    daf_plugin_view** const viewptr = new daf_plugin_view*;
    *viewptr = new daf_plugin_view(host, instancePointer, sampleRate);
