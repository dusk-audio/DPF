...
#ifdef DAF_OS_WINDOWS
# if DAF_IS_STANDALONE || defined(STATIC_BUILD)
...
const char* getPluginFormatName() noexcept
{
    ...
    return "Standalone";
   #endif
#elif defined(DAF_PLUGIN_TARGET_LADSPA)
    return "LADSPA";
    ...
    return "CLAP";
#elif defined(DAF_PLUGIN_TARGET_STATIC) && defined(DAF_PLUGIN_TARGET_STATIC_NAME)
    return DAF_PLUGIN_TARGET_STATIC_NAME;
#else
    return "Unknown";
#endif
}
...
void _createDirIfNeeded(const char* const dir)
{
    try {
        if (access(dir, F_OK) != 0)
            mkdir(dir, 0755);
    } DAF_SAFE_EXCEPTION("createDirIfNeeded");
}
#endif
...
{
   #if defined(DAF_OS_MAC) || defined(DAF_OS_WASM) || defined(DAF_OS_WINDOWS)
...
static const char* _getConfigDirForPlugin()
{
    ...
    {
        dir = _getConfigDir();
        dir += DAF_PLUGIN_NAME DAF_OS_SEP_STR;
        _createDirIfNeeded(dir);
    }
...
static const char* _getDocumentsDir()
{
    ...
    if (dir.isEmpty())
    {
        ...
        if (dir.isNotEmpty() && ! dir.endsWith(DAF_OS_SEP))
            dir += DAF_OS_SEP_STR;
    }
...
static const char* _getDocumentsDirForPlugin()
{
    ...
    if (dir.isEmpty())
    {
        ...
        if (SHGetFolderPathW(nullptr, CSIDL_MYDOCUMENTS, nullptr, SHGFP_TYPE_CURRENT, wpath) == S_OK)
        {
            wcscat(wpath, L"\\" DAF_PLUGIN_NAME "\\");
            _wmkdir(wpath);
...
{
    DAF_SAFE_ASSERT_RETURN(bundlePath != nullptr, nullptr);

   #if defined(DAF_PLUGIN_TARGET_AU) || defined(DAF_PLUGIN_TARGET_JACK) || defined(DAF_PLUGIN_TARGET_VST2) || defined(DAF_PLUGIN_TARGET_CLAP)
    ...
    if (resourcePath.isEmpty())
    {
       ...
       #else
        resourcePath += DAF_OS_SEP_STR "resources";
       #endif
    }
