 ...
 * - verify that latency changes works (with and without DAF_VST3_USES_SEPARATE_CONTROLLER)
 * == MIDI
 ...
 * == INFO
 * - set factory email (needs new DAF API, useful for LV2 as well)
...
#define DAF_VST3_MAX_BUFFER_SIZE 32768
#define DAF_VST3_MAX_SAMPLE_RATE 384000
#define DAF_VST3_MAX_LATENCY     DAF_VST3_MAX_SAMPLE_RATE * 10
...
typedef uint32_t daf_tuid[4];
#ifdef DAF_PROPER_CPP11_SUPPORT
static_assert(sizeof(v3_tuid) == sizeof(daf_tuid), "uid size mismatch");
#endif

// --------------------------------------------------------------------------------------------------------------------
// custom, constant uids related to DAF

// DO NOT CHANGE THESE FOUR CHARACTERS TO 'D','A','F',' '.
// They are hashed into every VST3 class UID this framework emits. The DPF -> DAF
// rename (2026-08-22) deliberately left them alone: changing them rewrites the
// UID of every already-shipped VST3, and every saved session that references one
// stops finding its plugin. This is the one place where the old name is load-
// bearing rather than cosmetic.
static constexpr const uint32_t daf_id_entry = d_cconst('D', 'P', 'F', ' ');
static constexpr const uint32_t daf_id_clas  = d_cconst('c', 'l', 'a', 's');
static constexpr const uint32_t daf_id_comp  = d_cconst('c', 'o', 'm', 'p');
static constexpr const uint32_t daf_id_ctrl  = d_cconst('c', 't', 'r', 'l');
static constexpr const uint32_t daf_id_proc  = d_cconst('p', 'r', 'o', 'c');
static constexpr const uint32_t daf_id_view  = d_cconst('v', 'i', 'e', 'w');

// --------------------------------------------------------------------------------------------------------------------
// plugin specific uids (values are filled in during plugin init)

#if defined(DAF_PLUGIN_BRAND_ID) && !defined(DAF_VST3_DONT_USE_BRAND_ID)
static constexpr const uint32_t daf_id_brand = d_cconst(STRINGIFY(DAF_PLUGIN_BRAND_ID));
#else
...
const char* tuid2str(const v3_tuid iid)
{
    ...
    if (std::memcmp(iid, daf_tuid_class, sizeof(daf_tuid)) == 0)
        return "{daf_tuid_class}";
    if (std::memcmp(iid, daf_tuid_component, sizeof(daf_tuid)) == 0)
        return "{daf_tuid_component}";
    if (std::memcmp(iid, daf_tuid_controller, sizeof(daf_tuid)) == 0)
        return "{daf_tuid_controller}";
    if (std::memcmp(iid, daf_tuid_processor, sizeof(daf_tuid)) == 0)
        return "{daf_tuid_processor}";
    if (std::memcmp(iid, daf_tuid_view, sizeof(daf_tuid)) == 0)
        return "{daf_tuid_view}";
...
// --------------------------------------------------------------------------------------------------------------------
// daf_plugin_view_create (implemented on UI side)

v3_plugin_view** daf_plugin_view_create(v3_host_application** host, void* instancePointer, double sampleRate);
...
class PluginVst3
{
    ...
    private:
        bool placeSorted(const int32_t sampleOffset) noexcept
        {
            ...
            else
            {
                ...
                DAF_SAFE_ASSERT_RETURN(event2 != nullptr, true);
   ...
   #endif // DAF_PLUGIN_WANT_MIDI_INPUT
...
public:
    ...
    {
        DAF_SAFE_ASSERT_INT_RETURN(mediaType == V3_AUDIO || mediaType == V3_EVENT, mediaType, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_INT_RETURN(busDirection == V3_INPUT || busDirection == V3_OUTPUT, busDirection, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_INT_RETURN(busIndex >= 0, busIndex, V3_INVALID_ARG);

       #if DAF_PLUGIN_NUM_INPUTS+DAF_PLUGIN_NUM_OUTPUTS > 0 || DAF_PLUGIN_WANT_MIDI_INPUT || DAF_PLUGIN_WANT_MIDI_OUTPUT
        ...
        if (mediaType == V3_AUDIO)
        {
            ...
            else
            {
                ...
                return V3_INVALID_ARG;
               #endif // DAF_PLUGIN_NUM_OUTPUTS
            }
           #else
            d_stderr("invalid bus, line %d", __LINE__);
            return V3_INVALID_ARG;
           #endif // DAF_PLUGIN_NUM_INPUTS+DAF_PLUGIN_NUM_OUTPUTS
        }
        else
        {
            if (busDirection == V3_INPUT)
    ...
    v3_result getState(v3_bstream** const stream)
    {
       ...
       #if DAF_PLUGIN_WANT_PROGRAMS
        {
            String tmpStr("__daf_program__\xff");
        ...
        if (stateCount != 0)
        {
            ...
            state += "__daf_state_end__\xff";
        }
       #endif

        if (paramCount != 0)
        {
            state += "__daf_parameters_begin__\xff";
            ...
            state += "__daf_parameters_end__\xff";
        }
        ...
        for (int32_t wrtntotal = 0, wrtn; wrtntotal < size; wrtntotal += wrtn)
        {
            ...
            DAF_SAFE_ASSERT_INT_RETURN(res == V3_OK, res, res);
            DAF_SAFE_ASSERT_INT_RETURN(wrtn > 0, wrtn, V3_INTERNAL_ERR);
        }
                                 ...
                                 v3_speaker_arrangement* const outputs, const int32_t numOutputs)
    {
       ...
       #if DAF_PLUGIN_NUM_OUTPUTS > 0
        DAF_SAFE_ASSERT_RETURN(numOutputs >= 0, V3_INVALID_ARG);
    ...
    v3_result getBusArrangement(const int32_t busDirection, const int32_t busIndex, v3_speaker_arrangement* const speaker) const noexcept
    {
        DAF_SAFE_ASSERT_INT_RETURN(busDirection == V3_INPUT || busDirection == V3_OUTPUT, busDirection, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_INT_RETURN(busIndex >= 0, busIndex, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_RETURN(speaker != nullptr, V3_INVALID_ARG);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(data->symbolic_sample_size == V3_SAMPLE_32, V3_INVALID_ARG);
        ...
        const float* inputs[DAF_PLUGIN_NUM_INPUTS != 0 ? DAF_PLUGIN_NUM_INPUTS : 1];
...

        {
            ...
            if (data->inputs != nullptr)
            {
                for (int32_t b = 0; b < data->num_input_buses; ++b) {
                    ...
                    {
                        DAF_SAFE_ASSERT_INT_BREAK(i < DAF_PLUGIN_NUM_INPUTS, i);
                        if (!fEnabledInputs[i] && i < DAF_PLUGIN_NUM_INPUTS) {
           ...
           #endif
            for (; i < std::max(1, DAF_PLUGIN_NUM_INPUTS); ++i)
                inputs[i] = fDummyAudioBuffer;
        }
    ...
    v3_result getParameterInfo(const int32_t rindex, v3_param_info* const info) const noexcept
    {
        ...
        DAF_SAFE_ASSERT_RETURN(rindex >= 0, V3_INVALID_ARG);

        // TODO hash the parameter symbol
        info->param_id = rindex;

      #if DAF_VST3_USES_SEPARATE_CONTROLLER || DAF_PLUGIN_WANT_LATENCY || DAF_PLUGIN_WANT_PROGRAMS
        switch (rindex)
        {
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        case kVst3InternalParameterBufferSize:
            info->flags = V3_PARAM_READ_ONLY | V3_PARAM_IS_HIDDEN;
            info->step_count = DAF_VST3_MAX_BUFFER_SIZE - 1;
        ...
        DAF_SAFE_ASSERT_UINT_RETURN(index < fParameterCount, index, V3_INVALID_ARG);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(normalized >= 0.0 && normalized <= 1.0, V3_INVALID_ARG);
       ...
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        case kVst3InternalParameterBufferSize:
            snprintf_i32_utf16(output, d_roundToIntPositive(normalized * DAF_VST3_MAX_BUFFER_SIZE), 128);
            return V3_OK;
        case kVst3InternalParameterSampleRate:
            snprintf_i32_utf16(output, d_roundToIntPositive(normalized * DAF_VST3_MAX_SAMPLE_RATE), 128);
            return V3_OK;
       #endif
       #if DAF_PLUGIN_WANT_LATENCY
        case kVst3InternalParameterLatency:
            snprintf_i32_utf16(output, d_roundToIntPositive(normalized * DAF_VST3_MAX_LATENCY), 128);
            return V3_OK;
       #endif
       #if DAF_PLUGIN_WANT_PROGRAMS
        case kVst3InternalParameterProgram:
    ...
    v3_result getParameterValueForString(const v3_param_id rindex, int16_t* const input, double* const output)
    {
       ...
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        case kVst3InternalParameterBufferSize:
            *output = static_cast<double>(std::atoi(ScopedUTF8String(input))) / DAF_VST3_MAX_BUFFER_SIZE;
            return V3_OK;
        case kVst3InternalParameterSampleRate:
            *output = std::atof(ScopedUTF8String(input)) / DAF_VST3_MAX_SAMPLE_RATE;
            return V3_OK;
       #endif
       #if DAF_PLUGIN_WANT_LATENCY
        case kVst3InternalParameterLatency:
            *output = std::atof(ScopedUTF8String(input)) / DAF_VST3_MAX_LATENCY;
            return V3_OK;
       #endif
       #if DAF_PLUGIN_WANT_PROGRAMS
        case kVst3InternalParameterProgram:
    ...
    {
        DAF_SAFE_ASSERT_RETURN(normalized >= 0.0 && normalized <= 1.0, 0.0);
       ...
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        case kVst3InternalParameterBufferSize:
            return std::round(normalized * DAF_VST3_MAX_BUFFER_SIZE);
        case kVst3InternalParameterSampleRate:
            return normalized * DAF_VST3_MAX_SAMPLE_RATE;
       #endif
       #if DAF_PLUGIN_WANT_LATENCY
        case kVst3InternalParameterLatency:
            return normalized * DAF_VST3_MAX_LATENCY;
       #endif
       #if DAF_PLUGIN_WANT_PROGRAMS
        case kVst3InternalParameterProgram:
        ...
        const uint32_t index = static_cast<uint32_t>(rindex - kVst3InternalParameterCount);
        DAF_SAFE_ASSERT_UINT2_RETURN(index < fParameterCount, index, fParameterCount, 0.0);
    ...
    double plainParameterToNormalized(const v3_param_id rindex, const double plain)
    {
       ...
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        case kVst3InternalParameterBufferSize:
            return std::max<double>(0.0, std::min<double>(1.0, plain / DAF_VST3_MAX_BUFFER_SIZE));
        case kVst3InternalParameterSampleRate:
            return std::max<double>(0.0, std::min<double>(1.0, plain / DAF_VST3_MAX_SAMPLE_RATE));
       #endif
       #if DAF_PLUGIN_WANT_LATENCY
        case kVst3InternalParameterLatency:
            return std::max<double>(0.0, std::min<double>(1.0, plain / DAF_VST3_MAX_LATENCY));
       #endif
       #if DAF_PLUGIN_WANT_PROGRAMS
        case kVst3InternalParameterProgram:
    ...
    double getParameterNormalized(const v3_param_id rindex)
    {
        ...
        if (
           #if !DAF_VST3_PURE_MIDI_INTERNAL_PARAMETERS
    ...
    v3_result setParameterNormalized(const v3_param_id rindex, const double normalized)
    {
        ...
        DAF_SAFE_ASSERT_UINT2_RETURN(rindex >= kVst3InternalParameterCount, rindex, kVst3InternalParameterCount, V3_INVALID_ARG);

       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        const uint32_t index = static_cast<uint32_t>(rindex - kVst3InternalParameterCount);
        DAF_SAFE_ASSERT_UINT2_RETURN(index < fParameterCount, index, fParameterCount, V3_INVALID_ARG);

        if (fIsComponent) {
            DAF_SAFE_ASSERT_RETURN(!fPlugin.isParameterOutputOrTrigger(index), V3_INVALID_ARG);
        }
    ...
    v3_result comp2ctrl_notify(v3_message** const message)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(msgid != nullptr, V3_INVALID_ARG);

        v3_attribute_list** const attrs = v3_cpp_obj(message)->get_attributes(message);
        DAF_SAFE_ASSERT_RETURN(attrs != nullptr, V3_INVALID_ARG);
   ...
   #endif // DAF_VST3_USES_SEPARATE_CONTROLLER

    // ----------------------------------------------------------------------------------------------------------------

    void ctrl2view_connect(v3_connection_point** const other)
    {
        DAF_SAFE_ASSERT(fConnectedToUI == false);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fConnectionFromCtrlToView != nullptr, V3_INTERNAL_ERR);
        ...
        DAF_SAFE_ASSERT_RETURN(fConnectedToUI, V3_INTERNAL_ERR);
        ...
        {
            DAF_SAFE_ASSERT_RETURN(fComponentHandler != nullptr, V3_INTERNAL_ERR);
            ...
            DAF_SAFE_ASSERT_INT_RETURN(res == V3_OK, res, res);
            DAF_SAFE_ASSERT_INT2_RETURN(rindex >= kVst3InternalParameterCount,
                                            rindex, fParameterCount, V3_INTERNAL_ERR);
            DAF_SAFE_ASSERT_INT2_RETURN(rindex < kVst3InternalParameterCount + fParameterCount,
                                            rindex, fParameterCount, V3_INTERNAL_ERR);

            res = v3_cpp_obj(attrs)->get_int(attrs, "started", &started);
            DAF_SAFE_ASSERT_INT_RETURN(res == V3_OK, res, res);
            DAF_SAFE_ASSERT_INT_RETURN(started == 0 || started == 1, started, V3_INTERNAL_ERR);
        ...
        {
           #if DAF_VST3_USES_SEPARATE_CONTROLLER
            DAF_SAFE_ASSERT_RETURN(fConnectionFromCompToCtrl != nullptr, V3_INTERNAL_ERR);
    ...
    v3_result notify_midi(v3_attribute_list** const attrs)
    {
        ...
        // known maximum size
        DAF_SAFE_ASSERT_UINT_RETURN(size == 3, size, V3_INTERNAL_ERR);
...
private:
   ...
   #endif
   #if DAF_PLUGIN_NUM_OUTPUTS > 0
    bool fEnabledOutputs[DAF_PLUGIN_NUM_OUTPUTS];
   #endif
   #if DAF_VST3_USES_SEPARATE_CONTROLLER
    ...
    v3_result getAudioBusInfo(const uint32_t busId, v3_bus_info* const info) const
    {
        ...
        if (busId < busInfo.groups)
        {
            ...
            DAF_SAFE_ASSERT_RETURN(numChannels != 0, V3_INTERNAL_ERR);
        }
        else
        {
    ...
    {
        DAF_SAFE_ASSERT_RETURN(portCount != 0, 0);
                                            ...
                                            const int32_t offset = 0)
    {
        ...
        v3_param_value_queue** const queue = v3_cpp_obj(outparamsptr)->add_param_data(outparamsptr,
                                                                                      &paramId, &index);
        DAF_SAFE_ASSERT_RETURN(queue != nullptr, false);
        DAF_SAFE_ASSERT_RETURN(v3_cpp_obj(queue)->add_point(queue, 0, normalized, &index) == V3_OK, false);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fHostApplication != nullptr, nullptr);
        ...
        DAF_SAFE_ASSERT_INT_RETURN(res == V3_TRUE, res, nullptr);
        DAF_SAFE_ASSERT_RETURN(msg != nullptr, nullptr);
    ...
    {
        DAF_CUSTOM_SAFE_ASSERT_ONCE_RETURN("MIDI output unsupported", fHostEventOutputHandle != nullptr, false);
...
static uint32_t V3_API daf_static_ref(void*) { return 1; }
static uint32_t V3_API daf_static_unref(void*) { return 0; }

// --------------------------------------------------------------------------------------------------------------------
// v3_funknown for classes with a single instance

template<class T>
static uint32_t V3_API daf_single_instance_ref(void* const self)
{
    return ++(*static_cast<T**>(self))->refcounter;
}

template<class T>
static uint32_t V3_API daf_single_instance_unref(void* const self)
{
    return --(*static_cast<T**>(self))->refcounter;
}
...
struct daf_component;

static std::vector<daf_component**> gComponentGarbage;

static uint32_t handleUncleanComponent(daf_component** const componentptr)
{
...
struct daf_edit_controller;

static std::vector<daf_edit_controller**> gControllerGarbage;

static uint32_t handleUncleanController(daf_edit_controller** const controllerptr)
{
...
// --------------------------------------------------------------------------------------------------------------------
// daf_comp2ctrl_connection_point

struct daf_comp2ctrl_connection_point : v3_connection_point_cpp {
    std::atomic_int refcounter;
    ScopedPointer<PluginVst3>& vst3;
    v3_connection_point** other;

    daf_comp2ctrl_connection_point(ScopedPointer<PluginVst3>& v)
        : refcounter(1),
          vst3(v),
          other(nullptr)
    {
        ...
        ref = daf_single_instance_ref<daf_comp2ctrl_connection_point>;
        unref = daf_single_instance_unref<daf_comp2ctrl_connection_point>;
    ...
    {
        daf_comp2ctrl_connection_point* const point = *static_cast<daf_comp2ctrl_connection_point**>(self);

        if (v3_tuid_match(iid, v3_funknown_iid) ||
            v3_tuid_match(iid, v3_connection_point_iid))
        {
            d_debug("daf_comp2ctrl_connection_point => %p %s %p | OK", self, tuid2str(iid), iface);
            ++point->refcounter;
            *iface = self;
            return V3_OK;
        }

        d_debug("daf_comp2ctrl_connection_point => %p %s %p | WARNING UNSUPPORTED", self, tuid2str(iid), iface);
    ...
    {
        d_debug("daf_comp2ctrl_connection_point::connect => %p %p", self, other);
        daf_comp2ctrl_connection_point* const point = *static_cast<daf_comp2ctrl_connection_point**>(self);
        DAF_SAFE_ASSERT_RETURN(point->other == nullptr, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_RETURN(point->other != other, V3_INVALID_ARG);
    ...
    {
        d_debug("daf_comp2ctrl_connection_point => %p %p", self, other);
        daf_comp2ctrl_connection_point* const point = *static_cast<daf_comp2ctrl_connection_point**>(self);
        DAF_SAFE_ASSERT_RETURN(point->other != nullptr, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_RETURN(point->other == other, V3_INVALID_ARG);
    ...
    static v3_result V3_API notify(void* const self, v3_message** const message)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(vst3 != nullptr, V3_NOT_INITIALIZED);

        v3_connection_point** const other = point->other;
        DAF_SAFE_ASSERT_RETURN(other != nullptr, V3_NOT_INITIALIZED);

        v3_attribute_list** const attrlist = v3_cpp_obj(message)->get_attributes(message);
        DAF_SAFE_ASSERT_RETURN(attrlist != nullptr, V3_INVALID_ARG);

        int64_t target = 0;
        const v3_result res = v3_cpp_obj(attrlist)->get_int(attrlist, "__daf_msg_target__", &target);
        DAF_SAFE_ASSERT_RETURN(res == V3_OK, res);
        DAF_SAFE_ASSERT_INT_RETURN(target == 1, target, V3_INTERNAL_ERR);
...
#if DAF_PLUGIN_HAS_UI
// --------------------------------------------------------------------------------------------------------------------
// daf_ctrl2view_connection_point

struct daf_ctrl2view_connection_point : v3_connection_point_cpp {
    ScopedPointer<PluginVst3>& vst3;
    v3_connection_point** other;

    daf_ctrl2view_connection_point(ScopedPointer<PluginVst3>& v)
        : vst3(v),
          other(nullptr)
    {
    ...
    static v3_result V3_API connect(void* const self, v3_connection_point** const other)
    {
        d_debug("daf_ctrl2view_connection_point::connect => %p %p", self, other);
        daf_ctrl2view_connection_point* const point = *static_cast<daf_ctrl2view_connection_point**>(self);
        DAF_SAFE_ASSERT_RETURN(point->other == nullptr, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_RETURN(point->other != other, V3_INVALID_ARG);
    ...
    static v3_result V3_API disconnect(void* const self, v3_connection_point** const other)
    {
        d_debug("daf_ctrl2view_connection_point::disconnect => %p %p", self, other);
        daf_ctrl2view_connection_point* const point = *static_cast<daf_ctrl2view_connection_point**>(self);
        DAF_SAFE_ASSERT_RETURN(point->other != nullptr, V3_INVALID_ARG);
        DAF_SAFE_ASSERT_RETURN(point->other == other, V3_INVALID_ARG);
    ...
    static v3_result V3_API notify(void* const self, v3_message** const message)
    {
        ...
        const v3_result res = v3_cpp_obj(attrlist)->get_int(attrlist, "__daf_msg_target__", &target);
        DAF_SAFE_ASSERT_RETURN(res == V3_OK, res);
        DAF_SAFE_ASSERT_INT_RETURN(target == 1 || target == 2, target, V3_INTERNAL_ERR);
...
struct daf_midi_mapping : v3_midi_mapping_cpp {
    daf_midi_mapping()
    {
        // v3_funknown, static
        query_interface = query_interface_midi_mapping;
        ref = daf_static_ref;
        unref = daf_static_unref;
...
struct daf_edit_controller : v3_edit_controller_cpp {
    std::atomic_int refcounter;
   #if DAF_PLUGIN_HAS_UI
    ScopedPointer<daf_ctrl2view_connection_point> connectionCtrl2View;
   #endif
   #if DAF_VST3_USES_SEPARATE_CONTROLLER
    ScopedPointer<daf_comp2ctrl_connection_point> connectionComp2Ctrl;
    ScopedPointer<PluginVst3> vst3;
   #else
    ScopedPointer<PluginVst3>& vst3;
    bool initialized;
   ...
   #if DAF_VST3_USES_SEPARATE_CONTROLLER
    daf_edit_controller(v3_host_application** const hostApp)
        : refcounter(1),
          vst3(nullptr),
   #else
    daf_edit_controller(ScopedPointer<PluginVst3>& v, v3_host_application** const hostApp, v3_host_application** const hostComp)
        : refcounter(1),
          vst3(v),
          initialized(false),
   #endif
    ...
    {
        d_debug("daf_edit_controller() with hostApplication %p", hostApplicationFromFactory);
    ...
    {
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
        ...
        if (v3_tuid_match(iid, v3_midi_mapping_iid))
        {
            ...
            static daf_midi_mapping midi_mapping;
            static daf_midi_mapping* midi_mapping_ptr = &midi_mapping;
        ...
        if (v3_tuid_match(iid, v3_connection_point_iid))
        {
            ...
            if (controller->connectionComp2Ctrl == nullptr)
                controller->connectionComp2Ctrl = new daf_comp2ctrl_connection_point(controller->vst3);
            else
    ...
    static uint32_t V3_API ref_edit_controller(void* const self)
    {
        ...
        d_debug("daf_edit_controller::ref => %p | refcount %i", self, refcount);
        return refcount;
    }

    static uint32_t V3_API unref_edit_controller(void* const self)
    {
        daf_edit_controller** const controllerptr = static_cast<daf_edit_controller**>(self);
        daf_edit_controller* const controller = *controllerptr;

        if (const int refcount = --controller->refcounter)
        {
            d_debug("daf_edit_controller::unref => %p | refcount %i", self, refcount);
            return refcount;
        }
        ...
        if (daf_comp2ctrl_connection_point* const point = controller->connectionComp2Ctrl)
        {
            if (const int refcount = point->refcounter)
            {
                unclean = true;
                d_stderr("DAF warning: asked to delete controller while component connection point still active (refcount %d)", refcount);
            }
        }
        ...
        d_debug("daf_edit_controller::unref => %p | refcount is zero, deleting everything now!", self);

        delete controller;
        delete controllerptr;
       #else
        d_debug("daf_edit_controller::unref => %p | refcount is zero, deletion will be done by component later", self);
       #endif
        return 0;
    }
    ...
    static v3_result V3_API initialize(void* const self, v3_funknown** const context)
    {
       ...
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        DAF_SAFE_ASSERT_RETURN(controller->vst3 == nullptr, V3_INVALID_ARG);
       #else
        ...
        d_debug("daf_edit_controller::initialize => %p %p | host %p", self, context, hostApplication);
    ...
    {
        d_debug("daf_edit_controller::terminate => %p", self);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        d_debug("daf_edit_controller::set_component_state => %p %p", self, stream);
    ...
    {
        d_debug("daf_edit_controller::set_state => %p %p", self, stream);

       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
        DAF_SAFE_ASSERT_RETURN(controller->vst3 != nullptr, V3_NOT_INITIALIZED);
       #endif
    ...
    {
        d_debug("daf_edit_controller::get_state => %p %p", self, stream);
    ...
    {
        // d_debug("daf_edit_controller::get_parameter_count => %p", self);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        // d_debug("daf_edit_controller::get_parameter_info => %p %i", self, param_idx);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    static v3_result V3_API get_parameter_string_for_value(void* self, v3_param_id index, double normalized, v3_str_128 output)
    {
        ...
        // d_debug("daf_edit_controller::get_parameter_string_for_value => %p %u %f %p", self, index, normalized, output);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        d_debug("daf_edit_controller::get_parameter_value_for_string => %p %u %p %p", self, index, input, output);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        d_debug("daf_edit_controller::normalised_parameter_to_plain => %p %u %f", self, index, normalized);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        d_debug("daf_edit_controller::plain_parameter_to_normalised => %p %u %f", self, index, plain);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    static double V3_API get_parameter_normalised(void* self, v3_param_id index)
    {
        ...
        PluginVst3* const vst3 = controller->vst3;
        DAF_SAFE_ASSERT_RETURN(vst3 != nullptr, 0.0);
    ...
    {
        // d_debug("daf_edit_controller::set_parameter_normalised => %p %u %f", self, index, normalized);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        d_debug("daf_edit_controller::set_component_handler => %p %p", self, handler);
        daf_edit_controller* const controller = *static_cast<daf_edit_controller**>(self);
    ...
    {
        d_debug("daf_edit_controller::create_view => %p %s", self, name);
        ...
        PluginVst3* const vst3 = controller->vst3;
        DAF_SAFE_ASSERT_RETURN(vst3 != nullptr, nullptr);

        d_debug("daf_edit_controller::create_view => %p %s | edit-ctrl %p, factory %p",
                self, name,
        ...
        v3_host_application** const host = controller->hostApplicationFromInitialize != nullptr
                                        ...
                                        #endif
                                         : controller->hostApplicationFromFactory;
        DAF_SAFE_ASSERT_RETURN(host != nullptr, nullptr);

        v3_plugin_view** const view = daf_plugin_view_create(host,
                                                             vst3->getInstancePointer(),
                                                             vst3->getSampleRate());
        DAF_SAFE_ASSERT_RETURN(view != nullptr, nullptr);
        ...
        if (v3_cpp_obj_query_interface(view, v3_connection_point_iid, &uiconn) == V3_OK)
        {
            ...
            controller->connectionCtrl2View = new daf_ctrl2view_connection_point(controller->vst3);
...
// --------------------------------------------------------------------------------------------------------------------
// daf_process_context_requirements

struct daf_process_context_requirements : v3_process_context_requirements_cpp {
    daf_process_context_requirements()
    {
        // v3_funknown, static
...
struct daf_audio_processor : v3_audio_processor_cpp {
    std::atomic_int refcounter;
    ScopedPointer<PluginVst3>& vst3;

    daf_audio_processor(ScopedPointer<PluginVst3>& v)
        : refcounter(1),
          vst3(v)
    {
        // v3_funknown, single instance
        query_interface = query_interface_audio_processor;
        ref = daf_single_instance_ref<daf_audio_processor>;
        unref = daf_single_instance_unref<daf_audio_processor>;
    ...
    {
        daf_audio_processor* const processor = *static_cast<daf_audio_processor**>(self);
        ...
        if (v3_tuid_match(iid, v3_process_context_requirements_iid))
        {
            ...
            static daf_process_context_requirements context_req;
            static daf_process_context_requirements* context_req_ptr = &context_req;
                                                 ...
                                                 v3_speaker_arrangement* const outputs, const int32_t num_outputs)
    {
        ...
        d_debug("daf_audio_processor::set_bus_arrangements => %p %p %i %p %i",
    ...
    {
        d_debug("daf_audio_processor::get_bus_arrangement => %p %s %i %p",
    ...
    {
        // NOTE runs during RT
        // d_debug("daf_audio_processor::can_process_sample_size => %i", symbolic_sample_size);
    ...
    {
        d_debug("daf_audio_processor::get_latency_samples => %p", self);
        daf_audio_processor* const processor = *static_cast<daf_audio_processor**>(self);

        PluginVst3* const vst3 = processor->vst3;
        DAF_SAFE_ASSERT_RETURN(vst3 != nullptr, 0);
    ...
    {
        d_debug("daf_audio_processor::setup_processing => %p %p", self, setup);
        daf_audio_processor* const processor = *static_cast<daf_audio_processor**>(self);

        PluginVst3* const vst3 = processor->vst3;
        DAF_SAFE_ASSERT_RETURN(vst3 != nullptr, V3_NOT_INITIALIZED);

        d_debug("daf_audio_processor::setup_processing => %p %p | %d %f", self, setup, setup->max_block_size, setup->sample_rate);
    ...
    {
        d_debug("daf_audio_processor::set_processing => %p %u", self, state);
        daf_audio_processor* const processor = *static_cast<daf_audio_processor**>(self);
    ...
    {
        // NOTE runs during RT
        // d_debug("daf_audio_processor::process => %p", self);
        daf_audio_processor* const processor = *static_cast<daf_audio_processor**>(self);
    ...
    {
        d_debug("daf_audio_processor::get_tail_samples => %p", self);
        daf_audio_processor* const processor = *static_cast<daf_audio_processor**>(self);
...
struct daf_component : v3_component_cpp {
    std::atomic_int refcounter;
    ScopedPointer<daf_audio_processor> processor;
   #if DAF_VST3_USES_SEPARATE_CONTROLLER
    ScopedPointer<daf_comp2ctrl_connection_point> connectionComp2Ctrl;
   #else
    ScopedPointer<daf_edit_controller> controller;
   #endif
    ScopedPointer<PluginVst3> vst3;
    v3_host_application** const hostApplicationFromFactory;
    v3_host_application** hostApplicationFromInitialize;

    daf_component(v3_host_application** const host)
        : refcounter(1),
          hostApplicationFromFactory(host),
          hostApplicationFromInitialize(nullptr)
    {
        d_debug("daf_component() with hostApplication %p", hostApplicationFromFactory);
    ...
    {
        daf_component* const component = *static_cast<daf_component**>(self);
        ...
        if (v3_tuid_match(iid, v3_audio_processor_iid))
        {
            ...
            if (component->processor == nullptr)
                component->processor = new daf_audio_processor(component->vst3);
            else
        ...
        if (v3_tuid_match(iid, v3_connection_point_iid))
        {
            ...
            if (component->connectionComp2Ctrl == nullptr)
                component->connectionComp2Ctrl = new daf_comp2ctrl_connection_point(component->vst3);
            else
        ...
        if (v3_tuid_match(iid, v3_edit_controller_iid))
        {
            ...
            if (component->controller == nullptr)
                component->controller = new daf_edit_controller(component->vst3,
    ...
    static uint32_t V3_API ref_component(void* const self)
    {
        ...
        d_debug("daf_component::ref => %p | refcount %i", self, refcount);
        return refcount;
    }

    static uint32_t V3_API unref_component(void* const self)
    {
        daf_component** const componentptr = static_cast<daf_component**>(self);
        daf_component* const component = *componentptr;

        if (const int refcount = --component->refcounter)
        {
            d_debug("daf_component::unref => %p | refcount %i", self, refcount);
            return refcount;
        }
        ...
        if (daf_audio_processor* const proc = component->processor)
        {
            if (const int refcount = proc->refcounter)
            {
                unclean = true;
                d_stderr("DAF warning: asked to delete component while audio processor still active (refcount %d)", refcount);
            }
        }

       #if DAF_VST3_USES_SEPARATE_CONTROLLER
        if (daf_comp2ctrl_connection_point* const point = component->connectionComp2Ctrl)
        {
            if (const int refcount = point->refcounter)
            {
                unclean = true;
                d_stderr("DAF warning: asked to delete component while connection point still active (refcount %d)", refcount);
            }
        }
       #else
        if (daf_edit_controller* const controller = component->controller)
        {
            if (const int refcount = controller->refcounter)
            {
                unclean = true;
                d_stderr("DAF warning: asked to delete component while edit controller still active (refcount %d)", refcount);
            }
        }
       #endif
        ...
        d_debug("daf_component::unref => %p | refcount is zero, deleting everything now!", self);
    ...
    static v3_result V3_API initialize(void* const self, v3_funknown** const context)
    {
        ...
        // check if already initialized
        DAF_SAFE_ASSERT_RETURN(component->vst3 == nullptr, V3_INVALID_ARG);
        ...
        d_debug("daf_component::initialize => %p %p | hostApplication %p", self, context, hostApplication);
    ...
    {
        d_debug("daf_component::terminate => %p", self);
        daf_component* const component = *static_cast<daf_component**>(self);
    ...
    {
        d_debug("daf_component::get_controller_class_id => %p", class_id);

        std::memcpy(class_id, daf_tuid_controller, sizeof(v3_tuid));
        return V3_OK;
    }

    static v3_result V3_API set_io_mode(void* const self, const int32_t io_mode)
    {
        d_debug("daf_component::set_io_mode => %p %i", self, io_mode);
    ...
    {
        // NOTE runs during RT
        // d_debug("daf_component::get_bus_count => %p %s %s",
        ...
        // d_debug("daf_component::get_bus_count returns %i", ret);
        return ret;
    }
    ...
    {
        d_debug("daf_component::get_bus_info => %p %s %s %i %p",
    ...
    {
        d_debug("daf_component::get_routing_info => %p %p %p", self, input, output);
        daf_component* const component = *static_cast<daf_component**>(self);
                                         ...
                                         const int32_t bus_idx, const v3_bool state)
    {
        ...
        // d_debug("daf_component::activate_bus => %p %s %s %i %u",
    ...
    {
        d_debug("daf_component::set_active => %p %u", self, state);
        daf_component* const component = *static_cast<daf_component**>(self);
    ...
    static v3_result V3_API set_state(void* const self, v3_bstream** const stream)
    {
        d_debug("daf_component::set_state => %p", self);
        daf_component* const component = *static_cast<daf_component**>(self);
    ...
    static v3_result V3_API get_state(void* const self, v3_bstream** const stream)
    {
        d_debug("daf_component::get_state => %p %p", self, stream);
        daf_component* const component = *static_cast<daf_component**>(self);
...
static const char* getPluginCategories()
{
    ...
    if (firstInit)
    {
       #ifdef DAF_PLUGIN_VST3_CATEGORIES
        categories = DAF_PLUGIN_VST3_CATEGORIES;
       #elif DAF_PLUGIN_IS_SYNTH
        ...
        DAF_SAFE_ASSERT(categories.isNotEmpty());
    }
...
struct daf_factory : v3_plugin_factory_cpp {
    std::atomic_int refcounter;
    ...
    ~daf_factory()
    {
        ...
        {
            d_debug("DAF notice: cleaning up previously undeleted controllers now");

            for (std::vector<daf_edit_controller**>::iterator it = gControllerGarbage.begin();
                it != gControllerGarbage.end(); ++it)
            {
                daf_edit_controller** const controllerptr = *it;
                daf_edit_controller* const controller = *controllerptr;
                delete controller;
                delete controllerptr;
            }
        ...
        {
            d_debug("DAF notice: cleaning up previously undeleted components now");

            for (std::vector<daf_component**>::iterator it = gComponentGarbage.begin();
                it != gComponentGarbage.end(); ++it)
            {
                daf_component** const componentptr = *it;
                daf_component* const component = *componentptr;
                delete component;
                delete componentptr;
            }
    ...
    {
        daf_factory* const factory = *static_cast<daf_factory**>(self);
    ...
    {
        daf_factory** const factoryptr = static_cast<daf_factory**>(self);
        daf_factory* const factory = *factoryptr;
    ...
    {
        d_debug("daf_factory::get_factory_info => %p", info);
    ...
    {
        d_debug("daf_factory::num_classes");
       #if DAF_VST3_USES_SEPARATE_CONTROLLER
    ...
    {
        d_debug("daf_factory::get_class_info => %i %p", idx, info);
        std::memset(info, 0, sizeof(*info));
        DAF_SAFE_ASSERT_RETURN(idx <= 2, V3_INVALID_ARG);
        ...
        if (idx == 0)
        {
            std::memcpy(info->class_id, daf_tuid_class, sizeof(v3_tuid));
            d_strncpy(info->category, "Audio Module Class", ARRAY_SIZE(info->category));
        }
        else
        {
            std::memcpy(info->class_id, daf_tuid_controller, sizeof(v3_tuid));
    ...
    {
        d_debug("daf_factory::create_instance => %p %s %s %p", self, tuid2str(class_id), tuid2str(iid), instance);
        daf_factory* const factory = *static_cast<daf_factory**>(self);
        ...
        // create component
        if (v3_tuid_match(class_id, *(const v3_tuid*)&daf_tuid_class) && (v3_tuid_match(iid, v3_component_iid) ||
                                                                          v3_tuid_match(iid, v3_funknown_iid)))
        {
            daf_component** const componentptr = new daf_component*;
            *componentptr = new daf_component(hostApplication);
        ...
        if (v3_tuid_match(class_id, *(const v3_tuid*)&daf_tuid_controller) && (v3_tuid_match(iid, v3_edit_controller_iid) ||
                                                                               v3_tuid_match(iid, v3_funknown_iid)))
        {
            daf_edit_controller** const controllerptr = new daf_edit_controller*;
            *controllerptr = new daf_edit_controller(hostApplication);
    ...
    {
        d_debug("daf_factory::get_class_info_2 => %i %p", idx, info);
        std::memset(info, 0, sizeof(*info));
        DAF_SAFE_ASSERT_RETURN(idx <= 2, V3_INVALID_ARG);

        info->cardinality = 0x7FFFFFFF;
       #if DAF_VST3_USES_SEPARATE_CONTROLLER || !DAF_PLUGIN_HAS_UI
    ...
    {
        d_debug("daf_factory::get_class_info_utf16 => %i %p", idx, info);
        std::memset(info, 0, sizeof(*info));
        DAF_SAFE_ASSERT_RETURN(idx <= 2, V3_INVALID_ARG);
       ...
       #endif
        d_strncpy(info->sub_categories, getPluginCategories(), ARRAY_SIZE(info->sub_categories));
        DAF_NAMESPACE::strncpy_utf16(info->name, sPlugin->getName(), ARRAY_SIZE(info->name));
        DAF_NAMESPACE::strncpy_utf16(info->vendor, sPlugin->getMaker(), ARRAY_SIZE(info->vendor));
        DAF_NAMESPACE::strncpy_utf16(info->version, getPluginVersion(), ARRAY_SIZE(info->version));
        DAF_NAMESPACE::strncpy_utf16(info->sdk_version, "VST 3.7.4", ARRAY_SIZE(info->sdk_version));
    ...
    {
        d_debug("daf_factory::set_host_context => %p %p", self, context);
        daf_factory* const factory = *static_cast<daf_factory**>(self);
...
{
    USE_NAMESPACE_DAF;
    daf_factory** const factoryptr = new daf_factory*;
    *factoryptr = new daf_factory;
...
bool ENTRYFNNAME(ENTRYFNNAMEARGS)
{
    ...
    if (bundlePath.isEmpty())
    {
        ...
        tmpPath.truncate(tmpPath.rfind(DAF_OS_SEP));
        tmpPath.truncate(tmpPath.rfind(DAF_OS_SEP));

        if (tmpPath.endsWith(DAF_OS_SEP_STR "Contents"))
        {
            tmpPath.truncate(tmpPath.rfind(DAF_OS_SEP));
            bundlePath = tmpPath;
