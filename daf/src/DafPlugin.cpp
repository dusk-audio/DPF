...
/* ------------------------------------------------------------------------------------------------------------
 * Static data, see DafPluginInternal.hpp */
...
/* ------------------------------------------------------------------------------------------------------------
 * Static fallback data, see DafPluginInternal.hpp */
    ...
    : pData(new PrivateData())
{
    ...
    pData->audioPorts = new AudioPortWithBusId[DAF_PLUGIN_NUM_INPUTS+DAF_PLUGIN_NUM_OUTPUTS];
   #endif

   #if defined(DAF_ABORT_ON_ERROR) || defined(DAF_RUNTIME_TESTING)
    #define DAF_ABORT abort();
   #else
    #define DAF_ABORT
   #endif
    ...
    if (programCount > 0)
    {
       ...
       #else
        d_stderr2("DAF warning: Plugins with programs must define `DAF_PLUGIN_WANT_PROGRAMS` to 1");
        DAF_ABORT
       #endif
    }
    ...
    if (stateCount > 0)
    {
       ...
       #else
        d_stderr2("DAF warning: Plugins with state must define `DAF_PLUGIN_WANT_STATE` to 1");
        DAF_ABORT
       #endif
    }
