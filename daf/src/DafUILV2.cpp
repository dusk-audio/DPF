...
public:
              ...
              bundlePath, dspPtr, scaleFactor, appType, bgColor, fgColor, appClassName)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(options != nullptr,);
    ...
    void lv2ui_port_event(const uint32_t rindex, const uint32_t bufferSize, const uint32_t format, const void* const buffer)
    {
        if (format == 0)
        {
            ...
            DAF_SAFE_ASSERT_RETURN(bufferSize == sizeof(float),)
        ...
        else if (format == fURIDs.atomEventTransfer)
        {
            ...
            else if (atom->type == fURIDs.atomObject && fUridUnmap != nullptr)
            {
                ...
                DAF_SAFE_ASSERT_RETURN(property != nullptr,);
                DAF_SAFE_ASSERT_RETURN(atomvalue != nullptr,);

                DAF_SAFE_ASSERT_RETURN(property->type == fURIDs.atomURID,);
                DAF_SAFE_ASSERT_RETURN(atomvalue->type == fURIDs.atomPath || atomvalue->type == fURIDs.atomString,);
                ...
                {
                    const LV2_URID daf_lv2_urid = ((const LV2_Atom_URID*)property)->body;
                    DAF_SAFE_ASSERT_RETURN(daf_lv2_urid != 0,);

                    const char* const daf_lv2_key = fUridUnmap->unmap(fUridUnmap->handle, daf_lv2_urid);
                    DAF_SAFE_ASSERT_RETURN(daf_lv2_key != nullptr,);

                    /*constexpr*/ const size_t reqLen = std::strlen(DAF_PLUGIN_URI "#");
                    DAF_SAFE_ASSERT_RETURN(std::strlen(daf_lv2_key) > reqLen,);

                    const char* const key   = daf_lv2_key + reqLen;
            ...
            {
                d_stdout("DAF :: received atom not handled :: %s",
...
private:
    ...
    const struct URIDs {
        ...
        URIDs(const LV2_URID_Map* const uridMap)
            : _uridMap(uridMap),
              dpfKeyValue(map(DAF_PLUGIN_LV2_STATE_PREFIX "KeyValueState")),
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fWriteFunction != nullptr,);
    ...
    void setState(const char* const key, const char* const value)
    {
        ...
        const uint32_t eventInPortIndex = DAF_PLUGIN_NUM_INPUTS + DAF_PLUGIN_NUM_OUTPUTS;
        ...
        DAF_SAFE_ASSERT_RETURN(atomBuf != nullptr,);
    ...
    bool fileRequest(const char* const key)
    {
        ...
        String daf_lv2_key(DAF_PLUGIN_URI "#");
        daf_lv2_key += key;

        const int r = fUiRequestValue->request(fUiRequestValue->handle,
                                        fUridMap->map(fUridMap->handle, daf_lv2_key.buffer()),
                                        fURIDs.atomPath,
                                        nullptr);

        d_stdout("UI file request %s %p => %s %i", key, fUiRequestValue, daf_lv2_key.buffer(), r);
...
static LV2UI_Handle lv2ui_instantiate(const LV2UI_Descriptor*,
                                      ...
                                      const LV2_Feature* const* const features)
{
    if (uri == nullptr || std::strcmp(uri, DAF_PLUGIN_URI) != 0)
    {
    ...
    if (const LV2_DirectAccess_Interface* const directAccess = (const LV2_DirectAccess_Interface*)extData->data_access(DAF_PLUGIN_LV2_STATE_PREFIX "direct-access"))
    ...
    if (options != nullptr)
    {
       ...
       #endif
        const LV2_URID uridClassName   = uridMap->map(uridMap->handle, "urn:daf:className");
...
static void lv2ui_write_function(LV2UI_Controller controller,
                                 ...
                                 const void*      buffer)
{
    DAF_SAFE_ASSERT_RETURN(buffer_size >= 1,);
    ...
    case kUriAtomEventTransfer:
        if (const LV2_Atom* const atom = static_cast<const LV2_Atom*>(buffer))
        {
            ...
            // if (kURIs[atom->type] == "urn:daf:KeyValueState")
            {
...
LV2UI_Handle modgui_init(const char* const className, _custom_param_set param_set, _custom_patch_set patch_set)
{
    ...
    DAF_SAFE_ASSERT_RETURN(className != nullptr, nullptr);
    ...
    if (kURIs.empty())
    {
        ...
        kURIs.push_back(DAF_PLUGIN_LV2_STATE_PREFIX "KeyValueState");
    }
    ...
    static LV2_Options_Option options[3] = {
        ...
        },
        {
            LV2_OPTIONS_INSTANCE,
            0,
            uridMap.map(uridMap.handle, "urn:daf:className"),
...
{
    static const constexpr uint32_t URI_PREFIX_LEN = sizeof(DAF_PLUGIN_URI);
    DAF_SAFE_ASSERT_RETURN(std::strncmp(uri, DAF_PLUGIN_URI "#", URI_PREFIX_LEN) == 0,);
    ...
    lv2ui_port_event(static_cast<ModguiHandle*>(handle)->handle,
                     DAF_PLUGIN_NUM_INPUTS + DAF_PLUGIN_NUM_OUTPUTS, // events input port
