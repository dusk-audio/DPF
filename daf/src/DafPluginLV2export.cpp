...
#ifdef DAF_PLUGIN_LICENSED_FOR_MOD
...
#ifndef DAF_PLUGIN_LV2_STATE_PREFIX
# define DAF_PLUGIN_LV2_STATE_PREFIX "urn:daf:"
#endif

#ifndef DAF_PLUGIN_MINIMUM_BUFFER_SIZE
# define DAF_PLUGIN_MINIMUM_BUFFER_SIZE 2048
#endif

#ifndef DAF_PLUGIN_AND_UI_IN_SINGLE_OBJECT
# define DAF_PLUGIN_AND_UI_IN_SINGLE_OBJECT DAF_PLUGIN_WANT_DIRECT_ACCESS
#endif

#ifndef DAF_PLUGIN_USES_MODGUI
# define DAF_PLUGIN_USES_MODGUI 0
#endif

#ifndef DAF_PLUGIN_USES_CUSTOM_MODGUI
# define DAF_PLUGIN_USES_CUSTOM_MODGUI 0
#endif
...
// DGL_FLAGS blocks in Makefile.base.mk and daf__add_dgl_system_libs in cmake/DAF-plugin.cmake -- both
...
// already declare in lv2ManifestUiExtensionData and implement in DafUILV2.cpp.
//
...
#  define DAF_LV2_UI_TYPE "BeUI"
# elif defined(DAF_OS_MAC)
#  define DAF_LV2_UI_TYPE "CocoaUI"
# elif defined(DAF_OS_WINDOWS)
#  define DAF_LV2_UI_TYPE "WindowsUI"
# elif defined(HAVE_X11)
#  define DAF_LV2_UI_TYPE "X11UI"
# else
#  define DAF_LV2_UI_TYPE "UI"
# endif
#else
# define DAF_LV2_UI_TYPE "UI"
#endif

#define DAF_LV2_USE_EVENTS_IN  (DAF_PLUGIN_WANT_MIDI_INPUT || DAF_PLUGIN_WANT_TIMEPOS || DAF_PLUGIN_WANT_STATE)
#define DAF_LV2_USE_EVENTS_OUT (DAF_PLUGIN_WANT_MIDI_OUTPUT || DAF_PLUGIN_WANT_STATE)
...
static void addAttribute(DAF_NAMESPACE::String& text,
...
void lv2_generate_ttl(const char* const basename)
{
    ...
    {
        bundlePath.truncate(bundlePath.rfind(DAF_OS_SEP));
    }
...

    {
...
#if DAF_PLUGIN_HAS_UI && DAF_PLUGIN_AND_UI_IN_SINGLE_OBJECT
        ...
        manifestString += "<" DAF_PLUGIN_URI ">\n";
        manifestString += "    a lv2:Plugin ;\n";
        manifestString += "    lv2:binary <" + pluginDLL + "." DAF_DLL_EXTENSION "> ;\n";
#if DAF_PLUGIN_USES_MODGUI
...
#if DAF_PLUGIN_HAS_UI
        manifestString += "<" DAF_UI_URI ">\n";
        manifestString += "    a ui:" DAF_LV2_UI_TYPE " ;\n";
        manifestString += "    ui:binary <" + pluginUI + "." DAF_DLL_EXTENSION "> ;\n";
# if DAF_PLUGIN_AND_UI_IN_SINGLE_OBJECT
...
# else // DAF_PLUGIN_AND_UI_IN_SINGLE_OBJECT
        manifestString += "    rdfs:seeAlso <" + uiTTL + "> .\n";
# endif // DAF_PLUGIN_AND_UI_IN_SINGLE_OBJECT
        manifestString += "\n";
#endif

#if DAF_PLUGIN_WANT_PROGRAMS
        const String presetSeparator(std::strstr(DAF_PLUGIN_URI, "#") != nullptr ? ":" : "#");
        ...
        for (uint32_t i = 0; i < plugin.getProgramCount(); ++i)
        {
            ...
            presetString  = "<" DAF_PLUGIN_URI + presetSeparator + "preset" + strBuf + ">\n";
            presetString += "    a pset:Preset ;\n";
            presetString += "    lv2:appliesTo <" DAF_PLUGIN_URI "> ;\n";
...

    {
        ...
        // header
#if DAF_LV2_USE_EVENTS_IN || DAF_LV2_USE_EVENTS_OUT
        ...
        for (uint32_t i=0, count=plugin.getStateCount(); i < count; ++i)
        {
            ...
            pluginString += "<" DAF_PLUGIN_URI "#" + plugin.getStateKey(i) + ">\n";
        ...
        // plugin
        pluginString += "<" DAF_PLUGIN_URI ">\n";
#ifdef DAF_PLUGIN_LV2_CATEGORY
        pluginString += "    a " DAF_PLUGIN_LV2_CATEGORY ", lv2:Plugin, doap:Project ;\n";
        ...
        if (hasHostVisibleState)
        {
            for (uint32_t i=0, count=plugin.getStateCount(); i < count; ++i)
            {
                ...
                if ((hints & kStateIsHostWritable) == kStateIsHostWritable)
                    pluginString += "    patch:writable <" DAF_PLUGIN_URI "#" + key + "> ;\n";
                else
                    pluginString += "    patch:readable <" DAF_PLUGIN_URI "#" + key + "> ;\n";
            }
            pluginString += "\n";
        }
#endif
...

        {
            ...
            for (uint32_t i=0; i < DAF_PLUGIN_NUM_INPUTS; ++i, ++portIndex)
            {
                ...
                {
                    pluginString += "        pg:group <" DAF_PLUGIN_URI "#portGroup_"
            ...
            for (uint32_t i=0; i < DAF_PLUGIN_NUM_OUTPUTS; ++i, ++portIndex)
            {
...
#if DAF_LV2_USE_EVENTS_IN
            pluginString += "    lv2:port [\n";
            pluginString += "        a lv2:InputPort, atom:AtomPort ;\n";
            pluginString += "        lv2:index " + String(portIndex) + " ;\n";
            pluginString += "        lv2:name \"Events Input\" ;\n";
            pluginString += "        lv2:symbol \"lv2_events_in\" ;\n";
            pluginString += "        rsz:minimumSize " + String(DAF_PLUGIN_MINIMUM_BUFFER_SIZE) + " ;\n";
...
# endif
        ...
        // Darkglass Anagram
       #ifdef DAF_PLUGIN_ABBREVIATION
        pluginString += "    dg:abbreviation \"" DAF_PLUGIN_ABBREVIATION "\" ;\n";
       #endif
       #ifdef DAF_PLUGIN_ANAGRAM_BLOCK_IMAGE_OFF
        pluginString += "    dg:blockImageOff <" DAF_PLUGIN_ANAGRAM_BLOCK_IMAGE_OFF "> ;\n";
       #endif
       #ifdef DAF_PLUGIN_ANAGRAM_BLOCK_IMAGE_ON
        pluginString += "    dg:blockImageOn <" DAF_PLUGIN_ANAGRAM_BLOCK_IMAGE_ON "> ;\n";
       #endif
       #if defined(DAF_PLUGIN_ABBREVIATION) || \
           defined(DAF_PLUGIN_ANAGRAM_BLOCK_IMAGE_OFF) || \
           defined(DAF_PLUGIN_ANAGRAM_BLOCK_IMAGE_ON)
        pluginString += "\n";
       #endif

       #ifdef DAF_PLUGIN_BRAND
        // MOD
        pluginString += "    mod:brand \"" DAF_PLUGIN_BRAND "\" ;\n";
        pluginString += "    mod:label \"" DAF_PLUGIN_NAME "\" ;\n\n";
       #endif
        ...
        if (const uint32_t portGroupCount = plugin.getPortGroupCount())
        {
            ...
            for (uint32_t i = 0; i < portGroupCount; ++i)
            {
                ...
                DAF_SAFE_ASSERT_CONTINUE(portGroup.groupId != kPortGroupNone);
                DAF_SAFE_ASSERT_CONTINUE(portGroup.symbol.isNotEmpty());

                pluginString += "\n<" DAF_PLUGIN_URI "#portGroup_" + portGroup.symbol + ">\n";
                isInput = isOutput = false;

#if DAF_PLUGIN_NUM_INPUTS > 0
                for (uint32_t i=0; i < DAF_PLUGIN_NUM_INPUTS && !isInput; ++i)
                    isInput = plugin.getAudioPort(true, i).groupId == portGroup.groupId;
#endif

#if DAF_PLUGIN_NUM_OUTPUTS > 0
                for (uint32_t i=0; i < DAF_PLUGIN_NUM_OUTPUTS && !isOutput; ++i)
   ...
   #if DAF_PLUGIN_USES_MODGUI && !DAF_PLUGIN_USES_CUSTOM_MODGUI
    {
        ...
        modguiString += "<" DAF_PLUGIN_URI ">\n";
        modguiString += "    modgui:gui [\n";
       #ifdef DAF_PLUGIN_BRAND
        modguiString += "        modgui:brand \"" DAF_PLUGIN_BRAND "\" ;\n";
       #endif
        modguiString += "        modgui:label \"" DAF_PLUGIN_NAME "\" ;\n";
...

    {
        ...
        jsString += "s.setAttribute('src',e.api_version>=3?f.get_custom_resource_filename('module.js'):('/resources/module.js?uri='+escape(\"" DAF_PLUGIN_URI "\")+'&r='+VERSION));\n";
        jsString += "s.setAttribute('type','text/javascript');\n";
        jsString += "s.onload=function(){\n";
        jsString += " Module_" DAF_PLUGIN_MODGUI_CLASS_NAME "({\n";
        jsString += " locateFile: function(p,_){return e.api_version>=3?f.get_custom_resource_filename(p):('/resources/'+p+'?uri='+escape(\"" DAF_PLUGIN_URI "\")+'&r='+VERSION)},\n";
...

    {
        ...
        iconFile << "<div class='" DAF_PLUGIN_MODGUI_CLASS_NAME " mod-pedal'>" << std::endl;
...

    {
        ...
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal{" << std::endl;
        stylesheetFile << " padding:0;" << std::endl;
        stylesheetFile << " margin:0;" << std::endl;
        stylesheetFile << " width:" + String(DAF_UI_DEFAULT_WIDTH) + "px;" << std::endl;
        stylesheetFile << " height:" + String(DAF_UI_DEFAULT_HEIGHT + 50) + "px;" << std::endl;
        ...
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .canvas_wrapper{" << std::endl;
        ...
        stylesheetFile << " width:" + String(DAF_UI_DEFAULT_WIDTH) + "px;" << std::endl;
        stylesheetFile << " height:" + String(DAF_UI_DEFAULT_HEIGHT) + "px;" << std::endl;
        stylesheetFile << " text-align:center;" << std::endl;
        stylesheetFile << " z-index:21;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "/*" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .canvas_wrapper:focus-within{" << std::endl;
        stylesheetFile << " z-index:21;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "*/" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-plugin-title{" << std::endl;
        stylesheetFile << " position:absolute;" << std::endl;
        stylesheetFile << " text-align:center;" << std::endl;
        stylesheetFile << " width:100%;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal h1{" << std::endl;
        stylesheetFile << " font-size:20px;" << std::endl;
        stylesheetFile << " font-weight:bold;" << std::endl;
        stylesheetFile << " line-height:50px;" << std::endl;
        stylesheetFile << " margin:0;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-control-group{" << std::endl;
        stylesheetFile << " position:absolute;" << std::endl;
        stylesheetFile << " left:5px;" << std::endl;
        stylesheetFile << " z-index:35;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-pedal-input," << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-pedal-output{" << std::endl;
        stylesheetFile << " top:75px;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-audio-input," << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-audio-output{" << std::endl;
        stylesheetFile << " margin-bottom:25px;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .jack-disconnected{" << std::endl;
        stylesheetFile << " top:0px!important;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-switch-image{" << std::endl;
        ...
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-switch-image.off{" << std::endl;
        stylesheetFile << " background-position: right center !important;" << std::endl;
        stylesheetFile << "}" << std::endl;
        stylesheetFile << "." DAF_PLUGIN_MODGUI_CLASS_NAME ".mod-pedal .mod-switch-image.on{" << std::endl;
   ...
   #endif // DAF_PLUGIN_USES_MODGUI && !DAF_PLUGIN_USES_CUSTOM_MODGUI
...
# endif
        ...
        DAF_CUSTOM_SAFE_ASSERT_RETURN("Programs require parameters or full state", valid, presetsFile.close());
...
# if DAF_PLUGIN_WANT_FULL_STATE
        for (uint32_t i=0; i<numStates; ++i)
        {
            ...
            presetString  = "<" DAF_PLUGIN_LV2_STATE_PREFIX + plugin.getStateKey(i) + ">\n";
...
# if DAF_PLUGIN_WANT_FULL_STATE
            ...
            for (uint32_t j=0; j<numStates; ++j)
            {
                ...
                if (plugin.getStateHints(j) & kStateIsHostReadable)
                    presetString += DAF_PLUGIN_URI "#";
                else
                    presetString += DAF_PLUGIN_LV2_STATE_PREFIX;
