...
#endif
#if DAF_PLUGIN_WANT_FULL_STATE && !defined(DAF_PLUGIN_WANT_FULL_STATE_WITH_LADSPA)
# error Cannot use full state with LADSPA or DSSI
#endif

#if DAF_PLUGIN_WANT_TIMEPOS && !defined(DAF_NO_WARNINGS)
...
# endif
# if DAF_PLUGIN_WANT_STATE && !defined(DAF_NO_WARNINGS)
...
#if DAF_PLUGIN_NUM_INPUTS > 0
        for (ulong i=0; i < DAF_PLUGIN_NUM_INPUTS; ++i)
        {
            if (port == index++)
            {
...
#if DAF_PLUGIN_NUM_OUTPUTS > 0
        for (ulong i=0; i < DAF_PLUGIN_NUM_OUTPUTS; ++i)
        {
            if (port == index++)
            {
...
#if defined(DAF_PLUGIN_TARGET_DSSI) && ! DAF_PLUGIN_WANT_MIDI_INPUT
        return; // unused
...
# if DAF_PLUGIN_WANT_PROGRAMS
    ...
    void dssi_select_program(const ulong bank, const ulong program)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(realProgram < fPlugin.getProgramCount(),);
...
private:
    ...
    // LADSPA ports
#if DAF_PLUGIN_NUM_INPUTS > 0
    const LADSPA_Data*  fPortAudioIns[DAF_PLUGIN_NUM_INPUTS];
#else
    const LADSPA_Data** fPortAudioIns;
#endif
#if DAF_PLUGIN_NUM_OUTPUTS > 0
    LADSPA_Data*  fPortAudioOuts[DAF_PLUGIN_NUM_OUTPUTS];
#else
...
static const struct DescriptorInitializer
{
    DescriptorInitializer()
    {
        ...
        // Get port count, init
        ulong port = 0;
        ulong portCount = DAF_PLUGIN_NUM_INPUTS + DAF_PLUGIN_NUM_OUTPUTS + plugin.getParameterCount();
#if DAF_PLUGIN_WANT_LATENCY
        portCount += 1;
#endif
        ...
        // Set ports
#if DAF_PLUGIN_NUM_INPUTS > 0
        for (ulong i=0; i < DAF_PLUGIN_NUM_INPUTS; ++i, ++port)
        {
...
#if DAF_PLUGIN_NUM_OUTPUTS > 0
        for (ulong i=0; i < DAF_PLUGIN_NUM_OUTPUTS; ++i, ++port)
        {
