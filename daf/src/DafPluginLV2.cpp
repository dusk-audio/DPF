          ...
          fWorker(worker)
    {
        ...
        if (const uint32_t count = fPlugin.getStateCount())
        {
            ...
            for (uint32_t i=0; i < count; ++i)
            {
                ...
                const String lv2key(DAF_PLUGIN_URI "#" + statekey);
...
#if DAF_PLUGIN_WANT_MIDI_INPUT || DAF_PLUGIN_WANT_TIMEPOS
...
# endif
        ...
        LV2_ATOM_SEQUENCE_FOREACH(fPortEventsIn, event)
        {
            ...
            if (event->body.type == fURIDs.dpfKeyValue)
            {
                ...
                if (std::strcmp((const char*)data, "__daf_ui_data__") == 0)
                {
                              ...
                              const LV2_Feature* const* const features)
    {
        ...
        for (uint32_t i=0, count=fPlugin.getStateCount(); i < count; ++i)
        {
            ...
            for (StringToStringMap::const_iterator cit=fStateMap.begin(), cite=fStateMap.end(); cit != cite; ++cit)
            {
                ...
                {
                    lv2key = DAF_PLUGIN_LV2_STATE_PREFIX;
    ...
    LV2_Worker_Status lv2_work(const void* const data)
    {
        ...
        if (eventBody->type == fURIDs.atomObject)
        {
            ...
            const LV2_Atom* value    = nullptr;
            lv2_atom_object_get(object, fURIDs.patchProperty, &property, fURIDs.patchValue, &value, 0);
            DAF_SAFE_ASSERT_RETURN(property != nullptr, LV2_WORKER_ERR_UNKNOWN);
            DAF_SAFE_ASSERT_RETURN(property->type == fURIDs.atomURID, LV2_WORKER_ERR_UNKNOWN);
            DAF_SAFE_ASSERT_RETURN(value != nullptr, LV2_WORKER_ERR_UNKNOWN);
            DAF_SAFE_ASSERT_RETURN(value->type == fURIDs.atomPath ||
...
private:
    ...
    // LV2 ports
   #if DAF_PLUGIN_NUM_INPUTS > 0
    const float*  fPortAudioIns[DAF_PLUGIN_NUM_INPUTS];
   #else
    const float** fPortAudioIns;
   #endif
   #if DAF_PLUGIN_NUM_OUTPUTS > 0
    float*  fPortAudioOuts[DAF_PLUGIN_NUM_OUTPUTS];
   #else
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fEventsOutData.port != nullptr, false);
...
#ifdef DAF_PLUGIN_LICENSED_FOR_MOD
    mod_license_check(features, DAF_PLUGIN_URI);
#endif
...
static const void* lv2_extension_data(const char* uri)
{
    ...
    if (std::strcmp(uri, DAF_PLUGIN_LV2_STATE_PREFIX "direct-access") == 0)
        return &directaccess;
#endif
