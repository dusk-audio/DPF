...
#ifdef DAF_RUNTIME_TESTING
# include "../extra/Thread.hpp"
#endif

#if defined(HAVE_JACK) && defined(STATIC_BUILD) && !defined(DAF_OS_WASM)
...
# if DAF_PLUGIN_NUM_OUTPUTS > 0
        for (uint32_t i=0; i < DAF_PLUGIN_NUM_OUTPUTS; ++i)
        {
            ...
            setAudioPortMetadata(port, fPortAudioOuts[i], DAF_PLUGIN_NUM_INPUTS+i);
        }
# endif
#endif
...
# endif
    ...
    {
#if DAF_PLUGIN_NUM_INPUTS > 0
        const float* audioIns[DAF_PLUGIN_NUM_INPUTS];
...
#if DAF_PLUGIN_NUM_OUTPUTS > 0
        float* audioOuts[DAF_PLUGIN_NUM_OUTPUTS];
...
#if DAF_PLUGIN_NUM_INPUTS > 0
    jack_port_t* fPortAudioIns[DAF_PLUGIN_NUM_INPUTS];
#endif
#if DAF_PLUGIN_NUM_OUTPUTS > 0
    jack_port_t* fPortAudioOuts[DAF_PLUGIN_NUM_OUTPUTS];
#endif
...
# endif
    ...
    {
        DAF_SAFE_ASSERT_RETURN(jackport != nullptr,);
...
# endif
    ...
    {
        DAF_SAFE_ASSERT_RETURN(index < fPlugin.getParameterCount(), false);
...
# endif
    ...
    {
        DAF_SAFE_ASSERT_RETURN(fPortMidiOutBuffer != nullptr, false);
...
protected:
    void run() override
    {
        ...
        for (int i=0; i<DAF_PLUGIN_NUM_INPUTS; ++i)
            inputs[i] = buffer;
        for (int i=0; i<DAF_PLUGIN_NUM_OUTPUTS; ++i)
            outputs[i] = buffer;
...
int main(int argc, char* argv[])
{
    ...
    if (bundlePath.isEmpty())
    {
      ...
      #else
       #ifdef DAF_OS_WINDOWS
        const DWORD attr = GetFileAttributesA(tmpPath + DAF_OS_SEP_STR "resources");
        if (attr != INVALID_FILE_ATTRIBUTES && (attr & FILE_ATTRIBUTE_DIRECTORY) != 0)
       #else
        if (access(tmpPath + DAF_OS_SEP_STR "resources", F_OK) == 0)
       #endif
        {
            bundlePath = tmpPath;
            d_nextBundlePath = bundlePath.buffer();
   ...
   #if defined(DAF_UI_LINUX_WEBVIEW_START)
    ...
    if (argc == 2 && std::strcmp(argv[1], "selftest") == 0)
    {
       ...
       #else
        d_stderr2("Code was built without DAF_RUNTIME_TESTING macro enabled, selftest option is not available");
        return 1;
       #endif
    }

   #if defined(DAF_OS_WINDOWS) && DAF_PLUGIN_HAS_UI
    ...
    jack_client_t* client = jackbridge_client_open(DAF_PLUGIN_NAME, JackNoStartServer, &status);
    ...
    if (client == nullptr)
    {
        ...
        CFStringRef errorTitleRef = CFStringCreateWithCString(nullptr,
           DAF_PLUGIN_NAME ": Error", kCFStringEncodingUTF8);
       ...
       #elif defined(DAF_OS_WINDOWS) && DAF_PLUGIN_HAS_UI
