...
public:
    ...
    intptr_t vst_dispatcher(const int32_t opcode, const int32_t index, const intptr_t value, void* const ptr, const float opt)
    {
        ...
        case VST_EFFECT_OPCODE_WINDOW_GETRECT:
            ...
            else
            {
                ...
                if (d_isZero(scaleFactor))
                    scaleFactor = 1.0;
                fVstRect.right = DAF_UI_DEFAULT_WIDTH * scaleFactor;
                fVstRect.bottom = DAF_UI_DEFAULT_HEIGHT * scaleFactor;
               #else
        ...
        case VST_EFFECT_OPCODE_SUPPORTS:
            ...
            {
               #if defined(DAF_OS_MAC) && DAF_PLUGIN_HAS_UI
        ...
        case VST_EFFECT_OPCODE_CUSTOM:
           #if DAF_PLUGIN_HAS_UI && !defined(DAF_OS_MAC)
    ...
    void updateParameterOutputsAndTriggers()
    {
        ...
        for (uint32_t i=0, count=fPlugin.getParameterCount(); i < count; ++i)
        {
            if (fPlugin.isParameterOutput(i))
            {
               ...
               #ifndef DAF_VST_SHOW_PARAMETER_OUTPUTS
...
ExtendedAEffect* getExtendedEffect(vst_effect* const effect)
{
    ...
    DAF_SAFE_ASSERT_RETURN(exteffect->valid == 101, nullptr);
    DAF_SAFE_ASSERT_RETURN(exteffect->audioMaster != nullptr, nullptr);
...
const vst_effect* VSTPluginMain(const vst_host_callback audioMaster)
{
   ...
   #ifdef DAF_VST_SHOW_PARAMETER_OUTPUTS
    ...
    for (uint32_t i=0, count=sPlugin->getParameterCount(); i < count; ++i)
    {
        if (sPlugin->isParameterInput(i))
        {
            ...
            DAF_SAFE_ASSERT_BREAK(! outputsReached);
            ++numParams;
            continue;
        }
        outputsReached = true;
    ...
    effect->num_inputs   = DAF_PLUGIN_NUM_INPUTS;
    effect->num_outputs  = DAF_PLUGIN_NUM_OUTPUTS;
...
#if !(defined(DAF_OS_MAC) || defined(DAF_OS_WASM) || defined(DAF_OS_WINDOWS) || (defined(DAF_OS_LINUX) && defined(DGL_USE_WEB_VIEW)))
DAF_PLUGIN_EXPORT
