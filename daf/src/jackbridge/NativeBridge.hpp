...
#if DAF_PLUGIN_NUM_INPUTS > 2
# define DAF_PLUGIN_NUM_INPUTS_2 2
#else
# define DAF_PLUGIN_NUM_INPUTS_2 DAF_PLUGIN_NUM_INPUTS
#endif

#if DAF_PLUGIN_NUM_OUTPUTS > 2
# define DAF_PLUGIN_NUM_OUTPUTS_2 2
#else
# define DAF_PLUGIN_NUM_OUTPUTS_2 DAF_PLUGIN_NUM_OUTPUTS
#endif

using DAF_NAMESPACE::HeapRingBuffer;
using DAF_NAMESPACE::RecursiveMutex;
using DAF_NAMESPACE::RecursiveMutexLocker;
...
struct NativeBridge {
    ...
    float* audioBuffers[DAF_PLUGIN_NUM_INPUTS + DAF_PLUGIN_NUM_OUTPUTS];
    ...
    {
        DAF_SAFE_ASSERT_RETURN(bufferSize != 0,);

        if (audio)
        {
           #if DAF_PLUGIN_NUM_INPUTS + DAF_PLUGIN_NUM_OUTPUTS > 0
            audioBufferStorage = new float[bufferSize*(DAF_PLUGIN_NUM_INPUTS+DAF_PLUGIN_NUM_OUTPUTS)];

            for (uint i=0; i<DAF_PLUGIN_NUM_INPUTS+DAF_PLUGIN_NUM_OUTPUTS; ++i)
                audioBuffers[i] = audioBufferStorage + (bufferSize * i);
           #endif

           #if DAF_PLUGIN_NUM_INPUTS > 0
            std::memset(audioBufferStorage, 0, sizeof(float)*bufferSize*DAF_PLUGIN_NUM_INPUTS);
           #endif
        }
    ...
    void* getPortBuffer(jack_port_t* const port)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(portMask != 0x0, nullptr);

       #if DAF_PLUGIN_NUM_INPUTS+DAF_PLUGIN_NUM_OUTPUTS > 0
        if (portMask & (kPortMaskAudio|kPortMaskCV))
            return audioBuffers[(portMask & kPortMaskInput ? 0 : DAF_PLUGIN_NUM_INPUTS) + (portMask & 0x0fff)];
       #endif
       #if DAF_PLUGIN_WANT_MIDI_INPUT
