...
struct SDL2Bridge : NativeBridge {
    ...
    bool open(const char* const clientName) override
    {
        ...
        requested.channels = DAF_PLUGIN_NUM_INPUTS_2;
        ...
        else if (receivedCapture.channels != DAF_PLUGIN_NUM_INPUTS_2)
        {
        ...
        requested.channels = DAF_PLUGIN_NUM_OUTPUTS_2;
        ...
        if (receivedPlayback.channels != DAF_PLUGIN_NUM_OUTPUTS_2)
        {
    ...
    bool close() override
    {
       ...
       #endif
       #if DAF_PLUGIN_NUM_OUTPUTS > 0
        DAF_SAFE_ASSERT_RETURN(playbackDeviceId != 0, false);
        SDL_CloseAudioDevice(playbackDeviceId);
        playbackDeviceId = 0;
       #endif
    ...
    static void AudioInputCallback(void* const userData, uchar* const stream, const int len)
    {
        ...
        // safety checks
        DAF_SAFE_ASSERT_RETURN(stream != nullptr,);
        DAF_SAFE_ASSERT_RETURN(len > 0,);

        if (self->jackProcessCallback == nullptr)
            return;

        const uint numFrames = static_cast<uint>(len / sizeof(float) / DAF_PLUGIN_NUM_INPUTS_2);
        DAF_SAFE_ASSERT_UINT2_RETURN(numFrames == self->bufferSize, numFrames, self->bufferSize,);
        ...
        for (uint i=0; i<DAF_PLUGIN_NUM_INPUTS_2; ++i)
        {
            for (uint j=0; j<numFrames; ++j)
                self->audioBuffers[i][j] = fstream[j * DAF_PLUGIN_NUM_INPUTS_2 + i];
        }
    ...
    static void AudioOutputCallback(void* const userData, uchar* const stream, const int len)
    {
        ...
        const uint numFrames = static_cast<uint>(len / sizeof(float) / DAF_PLUGIN_NUM_OUTPUTS_2);
        DAF_SAFE_ASSERT_UINT2_RETURN(numFrames == self->bufferSize, numFrames, self->bufferSize,);
        ...
        {
            for (uint j=0; j < numFrames; ++j)
                fstream[j * DAF_PLUGIN_NUM_OUTPUTS_2 + i] = self->audioBuffers[DAF_PLUGIN_NUM_INPUTS + i][j];
        }
    }
   #endif
};
