...
#ifndef DAF_RUNNER_HPP_INCLUDED
#define DAF_RUNNER_HPP_INCLUDED
...
#ifdef DAF_RUNNER_INDIRECT_WASM_CALLS
...
protected:
    ...
    {
        DAF_SAFE_ASSERT(! isRunnerActive());
...
public:
    ...
    {
       #ifndef DAF_OS_WASM
        DAF_SAFE_ASSERT_RETURN(!fRunnerThread.isThreadRunning(), false);
        fTimeInterval = timeIntervalMilliseconds;
        return fRunnerThread.startThread();
       #else
        DAF_SAFE_ASSERT_RETURN(fIntervalId == 0, false);
...
private:
    ...
    DAF_DECLARE_NON_COPYABLE(Runner)
};
