...
#if !defined(DAF_WEB_VIEW_HPP_INCLUDED) && !defined(DGL_WEB_VIEW_HPP_INCLUDED)
# error bad include
#endif
#if !defined(WEB_VIEW_DAF_NAMESPACE) && !defined(WEB_VIEW_DGL_NAMESPACE)
# error bad usage
#endif
...
#if defined(HAVE_X11) && defined(DAF_OS_LINUX)
...
#ifdef WEB_VIEW_DGL_NAMESPACE
using DAF_NAMESPACE::ChildProcess;
using DAF_NAMESPACE::RingBufferControl;
#endif
                            ...
                            const WebViewOptions& options)
{
    ...
    DAF_SAFE_ASSERT_RETURN(display != nullptr, nullptr);
    ...
    char** const envp = new char*[envsize + 5];
    {
        ...
        envp[e++] = ("DAF_WEB_VIEW_SCALE_FACTOR=" + String(scaleFactor)).getAndReleaseBuffer();
        envp[e++] = ("DAF_WEB_VIEW_WIN_ID=" +String(windowId)).getAndReleaseBuffer();
...
#define CSYM(S, NAME) \
    ...
    DAF_SAFE_ASSERT_RETURN(NAME != nullptr, false);
...
static int gtk3_js_cb(WebKitUserContentManager*, WebKitJavascriptResult* const result, void* const arg)
{
    ...
    DAF_SAFE_ASSERT_RETURN(string != nullptr, false);
                 ...
                 WebViewRingBuffer* const shmptr)
{
    ...
    DAF_SAFE_ASSERT_RETURN(webkit_web_view_evaluate_javascript != nullptr || webkit_web_view_run_javascript != nullptr, false);
    ...
    DAF_SAFE_ASSERT_RETURN(settings != nullptr, false);
    ...
    DAF_SAFE_ASSERT_RETURN(webview != nullptr, false);
                        ...
                        WebViewRingBuffer* const shmptr)
{
    ...
    DAF_SAFE_ASSERT_RETURN(QByteArray__init != nullptr, false);
    ...
    DAF_SAFE_ASSERT_RETURN(QString__init != nullptr, false);
    ...
    DAF_SAFE_ASSERT_RETURN(QWebEnginePage_setWebChannel != nullptr, false);
    ...
    if (qtVersion == 5) {
        ...
        DAF_SAFE_ASSERT_RETURN(QWebEnginePage_runJavaScript_compat != nullptr, false);
    } else {
        ...
        DAF_SAFE_ASSERT_RETURN(QWebEnginePage_runJavaScript != nullptr, false);
    }
...
int daf_webview_start(const int argc, char* argv[])
{
    if (argc != 3)
    {
    ...
    Display* const display = XOpenDisplay(nullptr);
    DAF_SAFE_ASSERT_RETURN(display != nullptr, 1);
    ...
    while (shmptr->valid && webview_timedwait(&shmptr->client.sem))
    {
        ...
        {
            DAF_SAFE_ASSERT_RETURN(rbctrl.readUInt() == kWebViewMessageInitData, 1);
