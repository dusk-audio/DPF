...
#ifndef DAF_BASE64_HPP_INCLUDED
#define DAF_BASE64_HPP_INCLUDED
...
{
    vector.clear();
    DAF_SAFE_ASSERT_RETURN(base64string != nullptr,);
    ...
    for (std::size_t l=0, len=std::strlen(base64string); l<len; ++l)
    {
        ...
        DAF_SAFE_ASSERT_CONTINUE(DafBase64Helpers::isBase64Char(c));
        ...
        {
            for (i=0; i<4; ++i)
                charArray4[i] = DafBase64Helpers::findBase64CharIndex(static_cast<char>(charArray4[i]));
    ...
    {
        for (j=0; j<i && j<4; ++j)
            charArray4[j] = DafBase64Helpers::findBase64CharIndex(static_cast<char>(charArray4[j]));
