...
#ifndef DAF_RING_BUFFER_HPP_INCLUDED
#define DAF_RING_BUFFER_HPP_INCLUDED
...
/**
   ...
   This struct details the data model used in DAF's RingBuffer class.

   DAF RingBuffer uses a struct just like this one to store positions, buffer data, size, etc.
...
public:
    ...
    {
        DAF_SAFE_ASSERT_RETURN(buffer != nullptr, false);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(buffer != nullptr, 0);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(buffer != nullptr,);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(data != nullptr, false);
        DAF_SAFE_ASSERT_RETURN(size > 0, false);
    ...
    bool commitWrite(const char* const debugMsg = nullptr) noexcept
    {
        ...
        // nothing to commit?
        if (debugMsg != nullptr) {
            DAF_CUSTOM_SAFE_ASSERT_RETURN(debugMsg, buffer->head != buffer->wrtn, false);
        } else {
            DAF_SAFE_ASSERT_RETURN(buffer->head != buffer->wrtn, false);
        }
    ...
    {
        DAF_SAFE_ASSERT_RETURN(buffer != ringBuf,);
...
protected:
    ...
    bool tryRead(void* const buf, const uint32_t size) noexcept
    {
       ...
       #endif
        DAF_SAFE_ASSERT_RETURN(buffer->buf != nullptr, false);
       #if defined(__clang__)
        #pragma clang diagnostic pop
       #endif
        DAF_SAFE_ASSERT_RETURN(buf != nullptr, false);
        DAF_SAFE_ASSERT_RETURN(size > 0, false);
        DAF_SAFE_ASSERT_RETURN(size < buffer->size, false);
    ...
    bool tryWrite(const void* const buf, const uint32_t size) noexcept
    {
        ...
        DAF_SAFE_ASSERT_RETURN(buf != nullptr, false);
        DAF_SAFE_ASSERT_RETURN(size > 0, false);
        DAF_SAFE_ASSERT_UINT2_RETURN(size < buffer->size, size, buffer->size, false);
...
private:
    ...
    DAF_PREVENT_VIRTUAL_HEAP_ALLOCATION
    DAF_DECLARE_NON_COPYABLE(RingBufferControl)
};
...
public:
    ...
    {
        DAF_SAFE_ASSERT_RETURN(heapBuffer.buf == nullptr, false);
        DAF_SAFE_ASSERT_RETURN(size > 0,  false);
        ...
        } DAF_SAFE_EXCEPTION_RETURN("HeapRingBuffer::createBuffer", false);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(heapBuffer.buf != nullptr,);
    ...
    {
        DAF_SAFE_ASSERT_RETURN(other.heapBuffer.size == heapBuffer.size,);
...
private:
    ...
    DAF_PREVENT_VIRTUAL_HEAP_ALLOCATION
    DAF_DECLARE_NON_COPYABLE(HeapRingBuffer)
};
...
private:
    ...
    DAF_PREVENT_VIRTUAL_HEAP_ALLOCATION
    DAF_DECLARE_NON_COPYABLE(SmallStackRingBuffer)
};
...
#endif // DAF_RING_BUFFER_HPP_INCLUDED
