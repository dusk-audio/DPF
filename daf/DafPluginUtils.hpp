...
#ifndef DAF_PLUGIN_UTILS_HPP_INCLUDED
#define DAF_PLUGIN_UTILS_HPP_INCLUDED
...
/**
   ...
   This path varies between systems and plugin formats, like so:
    ...
    - LV2: <bundle>/resources (can be stored anywhere inside the bundle really, DAF just uses this one)
...
struct AudioMidiSyncHelper
{
    ...
    float* outputs[DAF_PLUGIN_NUM_OUTPUTS];
    uint32_t frames;
    ...
    */
    AudioMidiSyncHelper(float** const o, uint32_t f, const MidiEvent* m, uint32_t mc)
        : outputs(),
          ...
          totalFramesUsed(0)
    {
        for (uint i=0; i<DAF_PLUGIN_NUM_OUTPUTS; ++i)
            outputs[i] = o[i];
    }
    ...
    */
    bool nextEvent()
    {
        ...
        if (totalFramesUsed == 0)
        {
            ...
            {
                DAF_SAFE_ASSERT_UINT2_RETURN(firstEventFrame < remainingFrames,
        ...
        DAF_SAFE_ASSERT_UINT2_RETURN(firstEventFrame >= totalFramesUsed,
...
#endif // DAF_PLUGIN_UTILS_HPP_INCLUDED
